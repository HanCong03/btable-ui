...
_p[7] = {
    value: function(require) {
        ...
        var PARSED_TYPE = _p.r(27);
        var ERROR = _p.r(12);
        var RangeAnalyzer = _p.r(25);
        var OperatorAnalyzer = _p.r(24);
        var currentTokens;
        return {
...
_p[15] = {
    value: function(require) {
        var TYPE = _p.r(27);
        var BOOLEAN_VALUE = _p.r(9);
        var ErrorCreator = _p.r(11);
        return {
            exec: function() {
...
_p[24] = {
    value: function(require) {
        var TYPE = _p.r(21).TYPE;
        var NEW_TYPE = _p.r(27);
        return {
...
_p[43] = {
    value: function(require) {
        ...
        return _p.r(0).create("DataAccess", $.extend({
            ...
            exchangeRow: function(index, range) {
                var map = this.kernelExchangeRow(index, range);
                var typeSource = this.data.type;
                var typeTarget = {};
                for (var key in map) {
                    if (map.hasOwnProperty(key)) {
                        typeTarget[key] = typeSource[map[key]];
                    }
                }
                $.extend(typeSource, typeTarget);
            },
...
_p[49] = {
    value: function(require) {
        var StructFacade = _p.r(48);
        var Parser = _p.r(47);
        var basic = _p.r(42);
        var LOCAL_CONFIG = _p.r(29);
...
_p[51] = {
    value: function(require) {
        var DEFAULT_FORMAT = _p.r(50);
        var TYPE_STYLE = _p.r(52);
...
_p[53] = {
    value: function(require) {
        var CELL_VALUE_TYPE = _p.r(63);
        var CELL_MODEL = _p.r(54);
        var BOOLEAN_VALUE = _p.r(62);
        var GeneralFormatter = _p.r(57);
        var NumericFormatter = _p.r(58);
        return {
            //getType: function (row, col) {
            //
        ...
        return _p.r(0).create("Configure", {
            ...
            getConfig: function(key) {
                ...
                for (var i = 0, len = key.length; i < len; i++) {
                    if (typeof conf[key[i]] !== "object") {
                        return conf[key[i]] || null;
                    }
                    conf = conf[key[i]];
                }
...
_p[67] = {
    value: function(require) {
        var Container = _p.r(70);
        var ElementFactory = _p.r(71);
        var Service = _p.r(74);
        var Command = _p.r(69);
        var Message = _p.r(72);
        var RefreshManager = _p.r(73);
        var Configure = _p.r(66);
        var ModuleLoader = _p.r(75);
        var PluginWrapper = _p.r(76);
            ...
            init: function() {
                ...
                this.containers = {
                    ...
                    scrollbarRight: this.createElement("btb-scrollbar-v-container", {
                        width: scrollbarConfig.width,
                        height: contentSize.height - borderWidth * 2,
                        top: headConfig.height,
                        right: 0,
...
_p[95] = {
    value: function(require) {
        var StyleCommand = _p.r(92);
        var ResizeCommand = _p.r(91);
        var ClearCommand = _p.r(89);
        var WriteCommand = _p.r(93);
        var ExportCommand = _p.r(90);
        return {
            write: WriteCommand,
...
_p[96] = {
    value: function(require) {
        ...
        var ESCAPE_MAP = _p.r(87);
        var UNESCAPE_MAP = _p.r(98);
        function indexToKey(row, col) {
        ...
        return {
            ...
            kernelGetValue: function(row, col) {
                return this.data.value[indexToKey(row, col)];
            },
            kernelGetFormat: function(row, col) {
                return this.data.format[indexToKey(row, col)];
            },
            kernelGetRowHeight: function(row) {
                return this.data.row[indexToKey(row, 0)];
            },
            kernelGetColumnWidth: function(col) {
                return this.data.column[indexToKey(0, col)];
            },
            ...
            kernelSetColumnWidth: function(value, col) {
                if (!value) {
                    this.data.column[indexToKey(0, col)] = undefined;
                } else {
                    this.data.column[indexToKey(0, col)] = Math.max(value, MIN_SIZE);
                }
            },
            kernelSetRowHeight: function(value, row) {
                if (!value) {
                    this.data.row[indexToKey(row, 0)] = undefined;
                } else {
                    this.data.row[indexToKey(row, 0)] = Math.max(value, MIN_SIZE);
                }
            },
            kernelExchangeRow: function(index, range) {
                ...
                data = this.data;
                // 原始数据
                var valueSource = data.value;
                var formatSource = data.format;
                var styleSource = data.style;
                // 排序中转
                var valueSorted = {};
                var formatSorted = {};
                var styleSorted = {};
                ...
                var styleKey;
                // 初始化style的中转对象
                for (var key in styleSource) {
                    if (styleSource.hasOwnProperty(key)) {
                        styleSorted[key] = {};
                        styleKeyset.push(key);
                    }
                }
                for (var i = 0, len = index.length; i < len; i++, startRow++) {
                    currentRow = index[i];
                    for (var j = startCol; j <= endCol; j++) {
                        targetIndex = indexToKey(startRow, j);
                        sourceIndex = indexToKey(currentRow, j);
                        map[targetIndex] = sourceIndex;
                        valueSorted[targetIndex] = valueSource[sourceIndex];
                        formatSorted[targetIndex] = formatSource[sourceIndex];
                        valueSource[sourceIndex] = undefined;
                        formatSource[sourceIndex] = undefined;
                        for (var k = 0, klen = styleKeyset.length; k < klen; k++) {
                            styleKey = styleKeyset[k];
                            styleSorted[styleKey][targetIndex] = styleSource[styleKey][sourceIndex];
                            styleSource[styleKey][sourceIndex] = undefined;
                        }
                    }
                }
                $.extend(valueSource, valueSorted);
                $.extend(formatSource, formatSorted);
                for (var i = 0, len = styleKeyset.length; i < len; i++) {
                    styleKey = styleKeyset[i];
                    for (var key in styleSorted[styleKey]) {
                        styleSource[styleKey][key] = styleSorted[styleKey][key];
                    }
                }
                return map;
            },
...
_p[108] = {
    value: function(require) {
        var $ = _p.r(2);
        var KEY_CODE = _p.r(109);
        var InputShadowMan = _p.r(107);
        var TextHelper = _p.r(110);
        var BORDER_WIDTH = 2;
        var PADDING = 2;
    ...
    value: function(require) {
        ...
        return _p.r(0).create("TextHelper", {
            ...
            init: function() {
                ...
                text.innerHTML = "&#xFEFF;";
                this.placeholder = text.firstChild;
            },
    ...
    value: function(require) {
        ...
        return _p.r(0).create("BodyScreen", {
            ...
            resize: function(width, height) {
                ...
                this.width = width;
                this.height = height;
                this.canvas.resize(width - 2 * borderWidth, height - 2 * borderWidth);
            },
            clear: function() {
                this.canvas.clear();
            },
            render: function() {
                ...
                for (var i = 0, len = count.row; i < len; i++) {
                    ...
                    for (var j = 0, jlen = count.col; j < jlen; j++) {
                        ...
                        contents[i][j] = struct.getDisplayValue(currentRow, currentCol);
                        widths[j] = colWidth[currentCol];
                        points[i][j] = {
                            x: colPoints[j] - offset,
                            y: rowPoints[i] - offset
                        };
                        styles[i][j] = struct.getComputedStyle(currentRow, currentCol);
                    }
                }
                var viewImage = painter.getViewImage(struct, contents, widths, heights, points, styles, defaultBgColor);
                canvas.drawImage(viewImage, borderWidth, borderWidth);
            }
        });
    }
};

//src/system/screen/body/cell-bg.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[115] = {
    value: function() {
        return '<div style="width: ${width}px; height: 100%; background: ${background}; position: absolute; top: 0; left: ${left}px;"></div>';
    }
};

//src/system/screen/body/center-content-bg.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[116] = {
    value: function() {
        return '<div style="width: ${width}px; height: 100%; position: absolute; top: 0; left: ${left}px; overflow: hidden; padding: ${pading}px;">' + '<div style="margin-left:${offset}px; display: inline-block; height: 100%;">' + '<span style="display: inline-block; transform: translateX(-50%); height: 100%; color: ${background}; background-color: ${background}">${content}</span>' + "</div>" + "</div>";
    }
};

//src/system/screen/body/center-content.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[117] = {
    value: function() {
        return '<div style="width: ${width}px; height: 100%; position: absolute; top: 0; left: ${left}px;">' + '<div style="width: ${contentWidth}px; height: ${contentHeight}px; position: absolute; top: ${padding}px; left: 2px; overflow: hidden;">' + '<table border="0" style="border-collapse: collapse; table-layout: fixed; width: 100%; height: 100%;">' + "<tbody>" + "<tr>" + '<td style="${style}; padding: 0; text-align: left;" valign="${verticalAlign}">' + '<div style="margin-left:${offset}px; display: inline-block;">' + '<span style="display: inline-block; transform: translateX(-50%)">${content}</span>' + "</div>" + "</td>" + "</tr>" + "</tbody>" + "</table>" + "</div>" + "</div>";
    }
};

//src/system/screen/body/content-bg.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[118] = {
    value: function() {
        return '<div style="width: ${width}px; height: 100%; position: absolute; top: 0; left: ${left}px; direction: ${direction}; overflow: hidden;">' + '<div style="${style}; padding: ${padding}px; color: ${background}; background-color: ${background}; display: inline-block; height: 100%;">' + "${content}" + "</div>" + "</div>";
    }
};
...
_p[119] = {
    value: function(require) {
        var $ = _p.r(2);
        var CONTENT = _p.r(120);
        var BG_CONTENT = _p.r(118);
        var BG_CELL = _p.r(115);
        var CENTER_CONTENT = _p.r(117);
        var CENTER_BG_CONTENT = _p.r(116);
        var PADDING = 2;
        var PATTERN = /\$\{([^\}]+)\}/g;
        var LOCAL_COFNIG = _p.r(29);
        var BORDER_WIDTH = _p.r(30).border.width;
        return _p.r(0).create("ContentPainter", {
            ...
            constructor: function(currentDocument, size) {
                ...
                this.doc = this.__initDoc(currentDocument, size);
                this.canvas = currentDocument.createElement("canvas");
                this.canvas.width = size.width;
                this.canvas.height = size.height;
                this.context = this.canvas.getContext("2d");
                this.image = new Image();
                this.__initSVGHead(size);
            },
            getViewImage: function(struct, contents, widths, heights, points, styles, defaultBgColor) {
                var contentView = this.getContentView(contents, widths, heights, points, styles);
                var cellBgView = this.getCellBgView(struct, widths, heights);
                var contentBgView = this.getContentBgView(contents, widths, heights, points, styles, defaultBgColor);
                this.__writeNewContent(contentBgView + cellBgView + contentView);
                this.svgContent[1] = this.__serialize();
                this.image.src = "data:image/svg+xml;base64," + this.toBase64(this.svgContent.join(""));
                this.context.clearRect(0, 0, this.size.width, this.size.height);
                this.context.drawImage(this.image, 0, 0);
                return this.canvas;
            },
            __initDoc: function(currentDocument, size) {
                var doc = currentDocument.implementation.createHTMLDocument();
                var body = doc.body;
                doc.documentElement.setAttribute("xmlns", doc.documentElement.namespaceURI);
                this.contentArea = body;
                this.$contentArea = $(this.contentArea);
                this.styleSerializer = currentDocument.createElement("div");
                this.$styleSerializer = $(this.styleSerializer);
            ...
            __initSVGHead: function(size) {
                this.svgContent[0] = "<svg xmlns='http://www.w3.org/2000/svg' width='" + size.width + "' height='" + size.height + "'>" + "<foreignObject width='" + size.width + "' height='" + size.height + "'>" + '<div xmlns="http://www.w3.org/1999/xhtml" style="font-size: 14px;">';
                this.svgContent[2] = "</div></foreignObject></svg>";
            },
            __writeNewContent: function(content) {
                this.contentArea.innerHTML = content;
            },
            __serialize: function() {
                return this.serializer.serializeToString(this.contentArea);
            },
            getContentView: function(contents, widths, heights, points, styles) {
                var cellLayout = this.getCellLayout(contents, styles);
                var currentRow;
                var currentBlock;
                var currentStyle;
                var colIndex;
                var htmls = [];
                var rowHtmls;
                for (var i = 0, len = cellLayout.length; i < len; i++) {
                    currentRow = cellLayout[i];
                    rowHtmls = [];
                    for (var j = 0, jlen = currentRow.length; j < jlen; j++) {
                        currentBlock = currentRow[j];
                        colIndex = currentBlock.index;
                        currentStyle = styles[i][colIndex];
                        switch (currentBlock.align) {
                          case "left":
                          case "right":
                            rowHtmls.push(this.getStandardCellFragment(currentBlock, contents[i][colIndex], widths, heights[i], currentStyle, points[i]));
                            break;

                          case "center":
                            rowHtmls.push(this.getCenterCellFragment(currentBlock, contents[i][colIndex], widths, heights[i], currentStyle, points[i]));
                            break;
                        }
                    }
                    htmls.push('<div style="width: 100%; height: ' + heights[i] + "px; position: absolute; top: " + points[i][0].y + 'px; left: 0; z-index: 3;">' + rowHtmls.join("") + "</div>");
                }
                return htmls.join("");
            },
            getCellBgView: function(struct, widths, heights) {
                var grid = struct.getGrid();
                var viewStart = struct.getViewStart();
                var colPoints = grid.col;
                var rowPoints = grid.row;
                var offset = struct.getOffset();
                // X 2
                var DIFF = struct.getBorderWidth() * 2;
                var rowHtmls = [];
                var colHtmls;
                var mapping;
                var style;
                for (var i = 0, len = heights.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = widths.length; j < jlen; j++) {
                        style = struct.getUserStyle(i + viewStart.row, j + viewStart.col);
                        mapping = {
                            width: widths[j] + DIFF,
                            left: colPoints[j] - offset,
                            background: style.backgroundColor
                        };
                        colHtmls[j] = BG_CELL.replace(PATTERN, function(match, name) {
                            return mapping[name];
                        });
                    }
                    rowHtmls[i] = '<div style="z-index: 2;height: ' + (heights[i] + DIFF) + "px; position: absolute; top: " + (rowPoints[i] - offset) + 'px;left: 0;">' + colHtmls.join("") + "</div>";
                }
                return rowHtmls.join("");
            },
            getContentBgView: function(contents, widths, heights, points, styles, defaultBgColor) {
                var cellLayout = this.getCellLayout(contents, styles);
                var currentRow;
                var currentBlock;
                var currentStyle;
                ...
                for (var i = 0, len = cellLayout.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = currentRow.length; j < jlen; j++) {
                        ...
                        switch (currentBlock.align) {
                          case "left":
                          case "right":
                            rowHtmls.push(this.getStandardCellBackgroundFragment(currentBlock, contents[i][colIndex], widths, heights[i], currentStyle, points[i]));
                            break;

                          case "center":
                            rowHtmls.push(this.getCenterCellBackgroundFragment(currentBlock, contents[i][colIndex], widths, heights[i], currentStyle, points[i]));
                            break;
                        }
                    }
                    htmls.push('<div style="width: 100%; height: ' + heights[i] + "px; position: absolute; top: " + points[i][0].y + 'px; left: 0; z-index: 1;">' + rowHtmls.join("") + "</div>");
                }
                return htmls.join("");
            },
            getCellLayout: function(contents, styles) {
                var layouts = [];
                var currentRow;
                var currentBlock;
                var count;
            ...
            getStandardCellFragment: function(block, content, widths, height, style, points) {
                if (!content) {
                    return "";
                }
                ...
                if (block.align === "left") {
                    ...
                    start = block.index - block.count;
                    end = block.index;
                }
                widths = widths.slice(start, end + 1);
                var width = sum(widths);
                ...
                var mapping = {
                    ...
                    left: points[start].x,
                    content: content,
                    style: this.getCssText(style),
                    direction: style.textAlign === "right" ? "rtl" : "ltr"
                };
                return CONTENT.replace(PATTERN, function(match, name) {
                    return mapping[name];
                });
            },
            getCenterCellFragment: function(block, content, widths, height, style, points) {
                if (!content) {
                    return "";
                }
                var start = block.index - block.left;
                var end = block.index + block.count;
                var columnWidths = widths.slice(start, end + 1);
                var width = sum(columnWidths);
                var contentWidth = width - 2 * PADDING;
                var mapping = {
                    width: width,
                    contentWidth: contentWidth,
                    ...
                    verticalAlign: style.verticalAlign,
                    left: points[start].x,
                    offset: points[block.index].x - points[start].x + widths[block.index] / 2,
                    content: content,
                    style: this.getCssText(style)
                };
                return CENTER_CONTENT.replace(PATTERN, function(match, name) {
                    return mapping[name];
                });
            },
            getStandardCellBackgroundFragment: function(block, content, widths, height, style, points) {
                if (!content) {
                    return "";
                }
                var start = block.index;
                ...
                var mapping = {
                    ...
                    style: this.getCssText(style),
                    direction: style.textAlign === "right" ? "rtl" : "ltr",
                    color: LOCAL_COFNIG.style.backgroundColor,
                    background: LOCAL_COFNIG.style.backgroundColor
                };
                return BG_CONTENT.replace(PATTERN, function(match, name) {
                    return mapping[name];
                });
            },
            getCenterCellBackgroundFragment: function(block, content, widths, height, style, points) {
                if (!content) {
                    return "";
                }
                var start = block.index - block.left;
                ...
                return CENTER_BG_CONTENT.replace(PATTERN, function(match, name) {
                    return mapping[name];
                });
            },
            getCssText: function(style) {
                this.styleSerializer.style.cssText = "";
                this.$styleSerializer.css(style);
                this.$styleSerializer.css({
                    background: "none"
                });
                return this.styleSerializer.style.cssText;
            }
        });
        function sum(values) {
            var sum = 0;
...
_p[120] = {
    value: function() {
        return '<div style="width: ${width}px; height: 100%; position: absolute; top: 0; left: ${left}px;">' + '<div style="width: ${contentWidth}px; height: ${contentHeight}px; position: absolute; top: ${padding}px; left: 2px; overflow: hidden;">' + '<table border="0" style="border-collapse: collapse; table-layout: fixed; width: 100%; height: 100%;">' + "<tbody>" + "<tr>" + '<td style="${style}; padding: 0; direction: ${direction};" valign="${verticalAlign}">${content}</td>' + "</tr>" + "</tbody>" + "</table>" + "</div>" + "</div>";
    }
};
...
_p[121] = {
    value: function(require) {
        return _p.r(0).create("Canvas", {
            ...
            node: null,
            context: null,
            constructor: function(doc, width, height, zoom) {
                this.zoom = zoom || 1;
    ...
    value: function(require) {
        ...
        return _p.r(0).create("BodyScreen", {
            ...
            render: function() {
                ...
                canvas.beginPath();
                // border box
                this.drawBoxBorder(offset, width, height);
            ...
            drawBoxBorder: function(offset, width, height) {
                var canvas = this.canvas;
                // left border
                canvas.moveTo(offset, offset);
                canvas.lineTo(offset, height);
                // top border
                canvas.moveTo(offset, offset);
                canvas.lineTo(width, offset);
                // bottom border
                canvas.moveTo(offset, this.height - offset);
                canvas.lineTo(this.width, this.height - offset);
                // right brder
                canvas.moveTo(this.width - offset, offset);
                canvas.lineTo(this.width - offset, this.height);
            }
        });
    }
};
...
_p[124] = {
    value: function(require) {
        var BodyScreen = _p.r(114);
        var TopScreen = _p.r(125);
        var LeftScreen = _p.r(123);
        var GridScreen = _p.r(122);
        return _p.r(0).create("Screen", {
            ...
            init: function() {
                ...
                this.bodyScreen.resize(size.width, size.height);
                this.gridScreen = this.createComponent(GridScreen);
                this.gridScreen.resize(size.width, size.height);
                size = this.getTopSize();
                this.topScreen = this.createComponent(TopScreen);
                this.topScreen.resize(size.width, size.height);
                size = this.getLeftSize();
                this.leftScreen = this.createComponent(LeftScreen);
                this.leftScreen.resize(size.width, size.height);
...
_p[127] = {
    value: function(require) {
        ...
        var ScrollEventManager = _p.r(126);
...
_p[130] = {
    value: function(require) {
        ...
        BTalbe.module(_p.r(99));
        BTalbe.mapEvent(_p.r(77));
        BTalbe.Interpreter = _p.r(94);
