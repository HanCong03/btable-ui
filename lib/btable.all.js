...
_p[120] = {
    value: function(require) {
        ...
        }, _p.r(108), _p.r(124), _p.r(126), _p.r(127), _p.r(121), _p.r(122), _p.r(129), _p.r(123), _p.r(125));
    }
};
    ...
    value: function(require) {
        ...
        return {
            kernelGetBorder: function(row, col) {
                var data = this.data;
                var borderData = data.border;
                var columnBorderData = data.columnBorder;
                ...
                for (var i = 0, len = BORDER_TYPE.length; i < len; i++) {
                    ...
                    value = borderData[borderType][index] || rowBorderData[borderType][row] || columnBorderData[borderType][col];
                    if (value && value !== NONE) {
            ...
            kernelSetAllBorder: function(borderOptions) {
                ...
                for (var i = 0, limit = totalIndex.col; i <= limit; i++) {
                    ...
                    for (var j = 0, jlen = BORDER_TYPE.length; j < jlen; j++) {
                        borderType = BORDER_TYPE[j];
            ...
            kernelSetRowBorder: function(borderOptions, startIndex, endIndex) {
                ...
                for (var i = startIndex; i <= endIndex; i++) {
                    currentBorderOptions = $._clone(borderOptions);
                    for (var j = 0, len = BORDER_TYPE.length; j < len; j++) {
                        borderType = BORDER_TYPE[j];
                ...
                if (startIndex - 1 >= 0) {
                    // 行清空
                    rowBorderData["bottom"][startIndex - 1] = NONE;
                    // 单元格清空
                    index = HASH.indexToKey(startIndex - 1, 0);
                    for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                        borderData["bottom"][index] = NONE;
                    }
                }
                /* ------- 清除下一行的数据 -------*/
                // 行清空
                rowBorderData["top"][endIndex + 1] = NONE;
                // 单元格清空
                index = HASH.indexToKey(endIndex + 1, 0);
                for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                    borderData["top"][index] = NONE;
                }
            },
            kernelSetColumnBorder: function(borderOptions, startIndex, endIndex) {
                ...
                if (currentColumnIndex >= 0) {
                    // 列清空
                    columnBorderData["right"][currentColumnIndex] = NONE;
                    // 单元格清空
                    for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                        index = HASH.indexToKey(0, currentColumnIndex);
                        borderData["right"][index] = NONE;
                    }
                }
                /* ------- 清除后一列的数据 -------*/
                currentColumnIndex = endIndex + 1;
                // 列清空
                columnBorderData["left"][currentColumnIndex] = NONE;
                // 单元格清空
                for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                    index = HASH.indexToKey(0, currentColumnIndex);
                    borderData["left"][index] = NONE;
                }
            ...
            __kernelSetRangeBorder: function(borderOptions, start, end) {
                ...
                for (var i = start.row, limit = end.row; i <= limit; i++) {
                    ...
                    for (var j = start.col, jlimit = end.col; j <= jlimit; j++, index++) {
                        currentBorderOptions = $._clone(borderOptions);
                        for (var k = 0, klen = BORDER_TYPE.length; k < klen; k++) {
                            borderType = BORDER_TYPE[k];
                            borderData[borderType][index] = currentBorderOptions[borderType];
                        }
                    }
                }
                // 清除上一行单元格数据
