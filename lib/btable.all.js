...
_p[37] = {
    value: function(require) {
        return _p.r(0).create("StyleCommand", {
            ...
            border: function(borderType, borderData, row, col) {
                var range;
                if (typeof borderType === "object") {
                    range = this.__getCells(borderData, row);
                    this.rs("c.set.borderdata", borderType, range.start, range.end);
                } else {
                    range = this.__getCells(row, col);
...
_p[50] = {
    value: function(require) {
        var basic = _p.r(49);
        var DataHelper = _p.r(51);
        // TODO 解决引用问题
        var KernelHelper = _p.r(102);
        var CONFIG = _p.r(33);
        return _p.r(0).create("DataAccess", $.extend({
            ...
            getDefaultStyle: function() {
                return $.extend({}, this.data.local, this.kernelGetDefaultStyle());
            },
...
_p[58] = {
    value: function(require) {
        return _p.r(0).create("Struct", {
            ...
            style: null,
            border: null,
            init: function(dataAccess, valueModule, style, border) {
                this.dataAccess = dataAccess;
                this.valueModule = valueModule;
                this.style = style;
                this.border = border;
...
_p[59] = {
    value: function(require) {
        ...
        var Struct = _p.r(0).create("Struct", {
            ...
            actuary: null,
            dataAccess: null,
            init: function(dataAccess, actuary, valueModule, style, border) {
                this.dataAccess = dataAccess;
                this.dataAccess.initLocalData(LOCAL_CONFIG);
                this.actuary = actuary;
                this.structFacade = this.createComponent(StructFacade, dataAccess, valueModule, style, border);
        ...
        Struct.deps = [ "dataAccess", "actuary", "value", "style", "border" ];
        return Struct;
    }
};
...
_p[61] = {
    value: function(require) {
        var DEFAULT_FORMAT = _p.r(60);
        var TYPE_STYLE = _p.r(62);
    ...
    value: function(require) {
         ...
         */
            analyze: function(content, formatType) {
                ...
                if (formatType === CELL_VALUE_TYPE.TEXT) {
                    return {
        ...
        return {
            analyze: function(content) {
                ...
                if (parsedValue) {
                    return {
                        type: CELL_VALUE_TYPE.NUMERIC,
                        originalType: CELL_VALUE_TYPE.DATE,
                ...
                if (parsedValue) {
                    return {
                        type: CELL_VALUE_TYPE.NUMERIC,
                        originalType: CELL_VALUE_TYPE.TIME,
...
_p[67] = {
    value: function(require) {
        var FormatAnalyzer = _p.r(63);
        var DateTimeConverter = _p.r(66);
        var NumericFormat = _p.r(69);
        var FORMAT_TYPE = _p.r(71);
            ...
            getNumericFormatValue: function(row, col) {
                ...
                switch (format) {
                  case FORMAT_TYPE.DATE:
                    return DateTimeConverter.numberToDate(rawValue);

                  case FORMAT_TYPE.TIME:
                    return DateTimeConverter.numberToTime(rawValue);

                  case FORMAT_TYPE.NUMERIC:
                    return NumericFormat.format(rawValue, this.dataAccess.getFormatParameter(row, col));
                }
            },
...
//src/core/value/format/numeric.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[69] = {
    value: function(require) {
        return {
            format: function(content, formatPrameter) {
                content = +content;
                if (formatPrameter && formatPrameter.precision !== undefined) {
                    content = content.toFixed(formatPrameter.precision);
                }
                content += "";
                if (!formatPrameter || !formatPrameter.thousandth) {
                    return content;
                }
                content = content.split(".");
                var decimal = content[1];
                if (decimal.length) {
                    decimal = "." + decimal;
                }
                ...
                lastIndex -= 3;
                while (lastIndex >= 0) {
                    content.splice(lastIndex + 1, 0, ",");
                    lastIndex -= 3;
                }
                return content.join("") + decimal;
            }
        };
    }
};
    ...
    value: function(require) {
        ...
        var Value = _p.r(0).create("Value", {
            ...
            writeCell: function(content, row, col) {
                ...
                if (formatData.originalType) {
                    this.dataAccess.setFormat(formatData.originalType, row, col);
                }
         ...
         */
            getComputedValueInfo: function(row, col) {
                ...
                } else if (valueType === CELL_VALUE_TYPE.NUMERIC) {
                    return {
                        type: CELL_VALUE_TYPE.NUMERIC,
...
_p[101] = {
    value: function(require) {
        var MAX_COLUMN_COUNT = _p.r(115).MAX_COLUMN_COUNT;
        return {
...
_p[112] = {
    value: function(require) {
        var StyleCommand = _p.r(109);
        var ResizeCommand = _p.r(108);
        var ClearCommand = _p.r(105);
        var WriteCommand = _p.r(110);
        var ExportCommand = _p.r(106);
        var PasteCommand = _p.r(107);
        return {
            write: WriteCommand,
...
_p[113] = {
    value: function(require) {
        var MIN_SIZE = 5;
        var ESCAPE_MAP = _p.r(100);
        var UNESCAPE_MAP = _p.r(117);
        var HASH = _p.r(101);
        var KernelHelper = _p.r(102);
        return $.extend({
            /* ---------------- read ---------------- */
         ...
         * @returns {*}
         */
            kernelExport: function(isSimple) {
                var data = this.data;
...
_p[115] = {
    value: {
        MAX_COLUMN_COUNT: Math.pow(26, 3),
        MAX_ROW_COUNT: Math.pow(2, 16)
    }
};
...
_p[123] = {
    value: function(require) {
        var StandardStyle = _p.r(125);
        var INLINE_ELEMENTS = _p.r(124);
        var ELEMENT_BLACKLIST = _p.r(119);
        var DEFAULT_ELEMENT_STYLE = _p.r(121);
...
_p[137] = {
    value: function(require) {
        var $ = _p.r(2);
        var KEY_CODE = _p.r(132);
        var InputShadowMan = _p.r(136);
        var TextHelper = _p.r(138);
        var BORDER_WIDTH = 2;
        var PADDING = 2;
...
_p[142] = {
    value: function(require) {
        var Canvas = _p.r(150);
        var ContentPainter = _p.r(147);
        var BorderPainter = _p.r(143);
        ...
        return _p.r(0).create("BodyScreen", {
            ...
            render: function() {
                ...
                this.borderPainter.draw(canvas, struct, userStyles, defaultBgColor);
            },
...
//src/system/screen/body/border-painter.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[143] = {
    value: function(require) {
        return _p.r(0).create("BorderPainter", {
            draw: function(canvas, struct, userStyles, defaultBgColor) {
                var viewStart = struct.getViewStart();
                var viewEnd = struct.getViewEnd();
                var grid = struct.getGrid();
                var colPoints = grid.col;
                ...
                for (var i = 0, len = borders.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = rowBorders.length; j < jlen; j++) {
                        currentBorderData = rowBorders[j];
                        bgcolor = userStyles.backgroundColor ? undefined : defaultBgColor;
                        if (!currentBorderData) {
                            continue;
                        }
                        if (currentBorderData.top) {
                            //canvas.beginPath();
                            //setBorderStyle(canvas, currentBorderData.top.style);
                            //canvas.strokeStyle(currentBorderData.top.color);
                            //canvas.moveTo(colPoints[j], rowPoints[i]);
                            //canvas.lineTo(colPoints[j + 1], rowPoints[i]);
                            //canvas.stroke();
                            setBorderStyle(canvas, "h", currentBorderData.top, {
                                x: colPoints[j],
                                y: rowPoints[i]
                            }, {
                                x: colPoints[j + 1],
                        ...
                        if (currentBorderData.left) {
                            setBorderStyle(canvas, "v", currentBorderData.left, {
                                x: colPoints[j],
                                y: rowPoints[i]
                            }, {
                                x: colPoints[j],
                        ...
                        if (currentBorderData.right) {
                            //    canvas.beginPath();
                            //    setBorderStyle(canvas, currentBorderData.right.style);
                            //    canvas.strokeStyle(currentBorderData.right.color);
                            //    canvas.moveTo(colPoints[j + 1], rowPoints[i]);
                            //    canvas.lineTo(colPoints[j + 1], rowPoints[i + 1]);
                            //    canvas.stroke();
                            setBorderStyle(canvas, "v", currentBorderData.right, {
                                x: colPoints[j + 1],
                                y: rowPoints[i]
                            }, {
                                x: colPoints[j + 1],
                        ...
                        if (currentBorderData.bottom) {
                            //    canvas.beginPath();
                            //    setBorderStyle(canvas, currentBorderData.bottom.style);
                            //    canvas.strokeStyle(currentBorderData.bottom.color);
                            //    canvas.moveTo(colPoints[j], rowPoints[i + 1]);
                            //    canvas.lineTo(colPoints[j + 1], rowPoints[i + 1]);
                            //    canvas.stroke();
                            setBorderStyle(canvas, "h", currentBorderData.bottom, {
                                x: colPoints[j],
                                y: rowPoints[i + 1]
                            }, {
                                x: colPoints[j + 1],
        ...
        function setBorderStyle(canvas, borderType, data, start, end, bgcolor) {
            switch (data.style) {
              case "thin":
                drawThin(canvas, data.color, start, end);
                break;

              case "dashed":
                drawDashed(canvas, 3, data.color, borderType, start, end, bgcolor);
                break;

              case "dotted":
                drawDashed(canvas, 1, data.color, borderType, start, end, bgcolor);
                break;
            }
        }
        function drawThin(canvas, color, start, end) {
            canvas.beginPath();
        ...
        function drawDashed(canvas, interval, color, borderType, start, end, bgcolor) {
            if (borderType === "h") {
                drawHDashed(canvas, interval, color, start.x, end.x, start.y, bgcolor);
            } else {
                drawVDashed(canvas, interval, color, start.y, end.y, start.x, bgcolor);
            }
        }
        function drawHDashed(canvas, interval, color, start, end, y, bgcolor) {
            var doubleInterval = 2 * interval;
            canvas.beginPath();
            canvas.strokeStyle(color);
            start -= .5;
            end -= .5;
            ...
            while (startPoint < end) {
                canvas.moveTo(startPoint, y);
                canvas.lineTo(Math.min(startPoint + interval, end), y);
                startPoint += doubleInterval;
            }
            canvas.stroke();
            if (!bgcolor) {
                return;
        ...
        function drawVDashed(canvas, interval, color, start, end, x, bgcolor) {
            var doubleInterval = 2 * interval;
            canvas.beginPath();
            canvas.strokeStyle(color);
            start -= .5;
            ...
            while (startPoint < end) {
                ...
                canvas.lineTo(x, Math.min(startPoint + interval, end));
                startPoint += doubleInterval;
            }
            canvas.stroke();
            if (!bgcolor) {
...
_p[147] = {
    value: function(require) {
        ...
        var ContentLayoutAnalyzer = _p.r(146);
        var ContentFillPainter = _p.r(145);
        var CellFillPainter = _p.r(144);
        var ContentPainter = _p.r(148);
...
_p[153] = {
    value: function(require) {
        var BodyScreen = _p.r(142);
        var TopScreen = _p.r(154);
        var LeftScreen = _p.r(152);
        var GridScreen = _p.r(151);
...
_p[156] = {
    value: function(require) {
        ...
        var ScrollEventManager = _p.r(155);
...
_p[160] = {
    value: function(require) {
        ...
        BTable.module(_p.r(118));
        BTable.mapEvent(_p.r(90));
        BTable.Interpreter = _p.r(111);
