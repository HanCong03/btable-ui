...
_p[4] = {
    value: function(require, exports, module) {
        var Context = _p.r(104);
        var FORMAT_CODE = _p.r(97);
        var codes = {};
...
_p[15] = {
    value: function(require) {
        var TAIL_SAPACE = _p.r(14).canvas.tailSpace;
        ...
        return {
            getCells: function(tokens) {
                ...
                for (var i = 0, len = tokens.length; i < len; i++) {
                    token = tokens[i];
                    if (token.type !== TOKEN_TYPE.OPERAND || token.subtype !== TOKEN_SUBTYPE.RANGE) {
                        continue;
                    }
                    currentCells = token.cells;
                    cells = cells.concat(token.cells);
                }
                // ------- 去重
                var current;
                var key;
...
_p[44] = {
    value: function(require) {
        var Parser = _p.r(57);
        var CellRelation = _p.r(43);
        var MathRuntime = _p.r(52);
        var Formula = _p.r(0).create("Formula", {
            ...
            getComputedValueInfo: function(row, col) {
                var value = this.dataAccess.getFormulaValue(row, col);
                if (value) {
                    return value;
                }
...
//src/core/formula/function/impl.js
/**
 * @file 解析器
 * @author hancong03@baiud.com
 */
            ...
            exec: function(args) {
                ...
                for (var i = 0, len = args.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = arg.length; j < jlen; j++) {
                        current = arg[j];
                        if (current.type !== CELL_TYPE.NUMBER) {
                            continue;
                        }
                        sum += +current.value;
                    }
...
_p[48] = {
    value: function(require) {
        var ERROR_VALUE = _p.r(96);
        var CELL_TYPE = _p.r(95);
        var TOKEN = _p.r(40);
        var TOKEN_SUBTYPE = TOKEN.SUBTYPE;
        var ComparisonRuntime = _p.r(49);
        var TypeCast = _p.r(53);
        return {
...
_p[50] = {
    value: function(require) {
        var ERROR_VALUE = _p.r(96);
        var CELL_TYPE = _p.r(95);
        var FunctionManager = _p.r(47);
        var OperandValue = _p.r(54);
        return {
            exec: function(valueModule, fun, operands) {
                ...
                operands = OperandValue.toValue(valueModule, operands);
                // 出错
                if (!$.isArray(operands)) {
                    return {
                        type: "RESULT",
...
_p[51] = {
    value: function(require) {
        ...
        var CELL_TYPE = _p.r(95);
        return {
            analyze: function(valueModule, stack) {
                if (stack.length !== 1) {
                    return {
                        type: CELL_TYPE.ERROR,
                        value: ERROR_VALUE.VALUE
                ...
                // 取出
                stack = stack[0];
                if (stack.type === "RESULT") {
                    return stack.result;
                }
                // 单元格
                if (stack.subtype !== TOKEN_SUBTYPE.RANGE) {
                    return {
                        type: stack.subtype,
                        value: stack.value
                    };
                ...
                var valueInfo = valueModule.getComputedValueInfo(cell.row, cell.col);
...
_p[52] = {
    value: function(require) {
        var TOKEN = _p.r(40);
        var TOKEN_TYPE = TOKEN.TYPE;
        var FunctionRuntime = _p.r(50);
        var BasicRuntime = _p.r(48);
        var ResultAnalyzer = _p.r(51);
        return _p.r(0).create("MathRuntime", {
            ...
            exec: function(tokens) {
                ...
                for (var i = 0, len = tokens.length; i < len; i++) {
                    ...
                    switch (token.type) {
                      ...
                      case TOKEN_TYPE.FUNCTION:
                        stack.push(FunctionRuntime.exec(this.valueModule, token, stack.splice(stack.length - token.argCount, token.argCount)));
                        break;

                      case TOKEN_TYPE.OP_IN:
                        stack.push(BasicRuntime.exec(this.valueModule, token, stack.splice(stack.length - 2, 2)));
                        break;

                      case TOKEN_TYPE.OP_PRE:
                      case TOKEN_TYPE.OP_POST:
                        stack.push(BasicRuntime.exec(this.valueModule, token, stack.splice(stack.length - 1, 1)));
                        break;
                ...
                // 分析最终结果
                return ResultAnalyzer.analyze(this.valueModule, stack);
            }
        });
    }
};
...
//src/core/formula/operand-value.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[54] = {
    value: function(require) {
        ...
        var TOKEN_SUBTYPE = TOKEN.SUBTYPE;
        return {
            toValue: function(valueModule, operands) {
                var current;
                var result = [];
                var currentResult;
                var value;
                for (var i = 0, len = operands.length; i < len; i++) {
                    ...
                    value = getCellsValue(valueModule, currentResult, current);
                    // error
                    if (value !== null) {
                        return value;
                    }
        ...
        /**
     * 提取引用的单元格集合的值，如果遇到读取的值是error类型，则返回该error值，且结束本次读取过程。
     * 如果过程中未遇到error类型的单元格，则返回null。
     */
        function getCellsValue(valueModule, result, cells) {
            var cell;
            var value;
            for (var i = 0, len = cells.length; i < len; i++) {
                cell = cells[i];
                if (cell.row === undefined) {
                    ...
                    value = valueModule.getComputedValueInfo(cell.row, cell.col);
                    if (value.type === CELL_TYPE.ERROR) {
                        return value;
                    }
                    result.push(value);
                }
...
_p[55] = {
     ...
     */
        function getIntersectCells(firstCell, secondCell) {
            firstCell = firstCell.cells;
            secondCell = secondCell.cells;
     ...
     */
        function getUnionCells(firstCell, secondCell) {
            firstCell.cells = firstCell.cells.concat(secondCell.cells);
            return firstCell;
        }
...
_p[56] = {
     ...
     */
        function processFunction(tokens, token, result) {
            ...
            fun.argCount = result.length - argsStart;
        }
        ...
        function getCells(text) {
            ...
            var tmp;
            if (groups.length === 1) {
                cells[0] = getCellInfo(groups[0]);
            } else {
                tmp = [];
                for (var i = 0, len = groups.length; i < len; i++) {
                    tmp[i] = [ getCellInfo(groups[i]) ];
                }
                cells = getRegionCellInfos(tmp);
            }
            var result = [];
            for (var i = 0, len = cells.length; i < len; i++) {
                tmp = cells[i];
                if (tmp.type === "region") {
                    if (tmp.start.row === undefined) {
                        for (var j = tmp.start.col, jlimit = tmp.end.col; j <= jlimit; j++) {
                            result.push({
                                row: undefined,
                                col: j
                            });
                        }
                    } else if (tmp.start.col === undefined) {
                        for (var j = tmp.start.row, jlimit = tmp.end.row; j <= jlimit; j++) {
                            result.push({
                                row: j,
                                col: undefined
                            });
                        ...
                        for (var j = tmp.start.row, jlimit = tmp.end.row; j <= jlimit; j++) {
                            for (var k = tmp.start.col, klimit = tmp.end.col; k <= klimit; k++) {
                                result.push({
                                    row: j,
                                    col: k
                                });
...
_p[57] = {
    value: function(require) {
        var TokenAnalyzer = _p.r(59);
        var CellsParser = _p.r(56);
        var RPNParser = _p.r(58);
        var CellReferenceParser = _p.r(55);
        return {
            parse: function(text) {
    ...
    value: function(require) {
         ...
         */
            getStyle: function(classify, row, col) {
                ...
                return $._extend($._clone(this.dataAccess.getDefaultStyle(classify)), typeStyle, userStyle);
            },
            getAllComputedStyle: function(row, col) {
...
_p[70] = {
    value: function(require) {
        var CELL_TYPE = _p.r(95);
        var NumberFormatter = _p.r(85);
        var LOGICAL_VALUE = _p.r(98);
        var CodeParser = _p.r(69);
...
_p[72] = {
    value: function(require) {
        var CELL_TYPE = _p.r(95);
        var NumberAnalyzer = _p.r(75);
        var LogicalAnalyzer = _p.r(74);
        var ErrorAnalyzer = _p.r(73);
        return {
            exec: function(content, numberFormatCode) {
                var result;
                if (!content) {
...
_p[75] = {
    value: function(require) {
        var NumericAnalyzer = _p.r(79);
        var DateTimeAnalyzer = _p.r(77);
        var CurrencyAnalyzer = _p.r(76);
        var PercentAnalyzer = _p.r(80);
        var FractionAnalyzer = _p.r(78);
        var CodeParser = _p.r(69);
        var CODE_FLAG = _p.r(68);
        return {
...
_p[85] = {
    value: function(require) {
        var CodeParser = _p.r(69);
        var DecimalFormatter = _p.r(87);
        var ScientificFormatter = _p.r(89);
        var DateTimeFormatter = _p.r(86);
        var FractionFormatter = _p.r(88);
        var CODE_FLAG = _p.r(68);
        var FORMAT_CODE = _p.r(97);
        return {
...
_p[91] = {
    value: function(require) {
        ...
        var NUMBER_FORMAT = _p.r(100);
        var Format = _p.r(82);
        var Formatter = _p.r(84);
        var ColorManager = _p.r(70);
        var Value = _p.r(0).create("Value", {
            ...
            colorManager: null,
            base: _p.r(119),
            init: function(dataAccess, formulaModule) {
                this.dataAccess = dataAccess;
...
_p[104] = {
    value: function(require) {
        var Container = _p.r(107);
        var ElementFactory = _p.r(108);
        var Service = _p.r(111);
        var Command = _p.r(106);
        var Message = _p.r(109);
        var RefreshManager = _p.r(110);
        var Configure = _p.r(103);
        var ModuleLoader = _p.r(112);
        var PluginWrapper = _p.r(113);
...
_p[126] = {
    value: function(require) {
        ...
        var STYLE_GROUP = _p.r(157).styleGroup;
        var DEFAULT_STYLE_VALUE = _p.r(129);
        var FORMAT_CODE = _p.r(97);
        var VALUE_FILTER = _p.r(125);
        var FORMAT_CODE_MAP = {};
        for (var key in FORMAT_CODE) {
            if (FORMAT_CODE.hasOwnProperty(key) && key !== "_default") {
                FORMAT_CODE_MAP[FORMAT_CODE[key]] = key;
...
_p[130] = {
    value: function(require) {
        var MAX_COLUMN_COUNT = _p.r(153).MAX_COLUMN_COUNT;
        return {
...
_p[141] = {
    value: function(require) {
        var StyleCommand = _p.r(138);
        var ResizeCommand = _p.r(137);
        var ClearCommand = _p.r(134);
        var WriteCommand = _p.r(139);
        var ExportCommand = _p.r(135);
        var PasteCommand = _p.r(136);
        return {
            write: WriteCommand,
...
_p[142] = {
    value: function(require) {
        ...
        }, _p.r(130), _p.r(151), _p.r(148), _p.r(144), _p.r(149), _p.r(152), _p.r(147), _p.r(143), _p.r(145), _p.r(146), _p.r(149), _p.r(143));
    }
};
...
_p[149] = {
    value: function(require) {
        var StyleUtil = _p.r(150);
        var APPLY_CLASSIFY = _p.r(155).applyName;
        return {
...
_p[152] = {
    value: function(require) {
        ...
        var XF_STYLE_DEFAULT = _p.r(154).xfStyle;
        return {
...
_p[163] = {
    value: function(require) {
        var StandardStyle = _p.r(165);
        var INLINE_ELEMENTS = _p.r(164);
        var ELEMENT_BLACKLIST = _p.r(159);
        var DEFAULT_ELEMENT_STYLE = _p.r(161);
        var BASE_FONT_SIZE = 13;
...
_p[182] = {
    value: function(require) {
        ...
        }, _p.r(181), _p.r(177), _p.r(178), _p.r(180)));
    }
};
...
_p[192] = {
    value: function(require) {
        var ContentFillPainter = _p.r(194);
        var ContentPainter = _p.r(195);
        var MergeFillPainter = _p.r(196);
        var CellFillPainter = _p.r(193);
        var GridPainter = _p.r(190);
        var BorderPainter = _p.r(188);
        var BoxPainter = _p.r(189);
        var PaneLinePainter = _p.r(191);
        return {
...
_p[199] = {
    value: function(require) {
        var BodyScreen = _p.r(186);
        var TopScreen = _p.r(200);
        var LeftScreen = _p.r(198);
...
_p[203] = {
    value: function(require) {
        ...
        var ScrollEventManager = _p.r(202);
...
_p[207] = {
    value: function(require) {
        var $ = _p.r(2);
        var Util = _p.r(179);
        var NormalWorker = _p.r(205);
        var PaneWorker = _p.r(206);
...
_p[208] = {
    value: function(require) {
        var $ = _p.r(2);
        var CELL_VALUE_TYPE = _p.r(95);
        var BOOLEAN_VALUE = _p.r(98);
        return _p.r(0).create("Cell", {
            base: _p.r(119),
            init: function() {
                this.initService();
...
_p[210] = {
    value: function(require) {
        ...
        BTable.module(_p.r(158));
        BTable.mapEvent(_p.r(114));
        BTable.Interpreter = _p.r(140);
        BTable.ExcelAdapter = _p.r(123);
