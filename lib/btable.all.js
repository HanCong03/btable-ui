...
_p[4] = {
    value: function(require, exports, module) {
        var Context = _p.r(132);
        var FORMAT_CODE = _p.r(125);
    ...
    value: function(require) {
        ...
        var Actuary = _p.r(0).create("Actuary", {
            ...
            getNormalGrid: function(paneGrid) {
                ...
                // 行下界溢出值
                var rowBottomSpace = spaceHeight - (inc + offset);
                var rowOverflow = rowBottomSpace > TAIL_SAPACE + (viewHeight[viewHeight.length - 1] || 0) + LINE_WIDTH;
                inc = offset;
                ...
                var colBottomSpace = spaceWidth - (inc + offset);
                var colOverflow = colBottomSpace > TAIL_SAPACE + (viewWidth[viewWidth.length - 1] || 0) + LINE_WIDTH;
                return {
                    row: rowPoints,
                    col: colPoints,
                    width: colWidths,
                    height: rowHeights,
                    startRow: viewStart.row,
                    startCol: viewStart.col,
                    rowMap: rowMap,
                    colMap: colMap,
                    rMap: rMap,
                    cMap: cMap,
                    ...
                    rowInEnd: rowBottomSpace > TAIL_SAPACE,
                    colInEnd: colBottomSpace > TAIL_SAPACE,
                    rowInStart: viewStart.row === 0,
                    colInStart: viewStart.col === 0
                };
            },
            reverseRow: function() {
            ...
            __patchRow: function(grid) {
                ...
                var space = this.dataAccess.getSpace();
                var offset = this.dataAccess.getOffset();
                var totalRow = this.dataAccess.getTotalIndex().row;
                var borderWidth = this.dataAccess.getBorderWidth();
                var heightList;
                if (paneGrid.rowIndex !== -1) {
                    heightList = this.__getRowPatchInfo(space.height - (grid.viewRowPoints[paneGrid.rowIndex + 1] + offset) - TAIL_SAPACE, totalRow);
                } else {
                    heightList = this.__getRowPatchInfo(space.height - borderWidth - TAIL_SAPACE, totalRow);
                }
                var originalStartRow = grid.startRow;
                grid.startRow = totalRow + 1 - heightList.length - (paneGrid.rowIndex + 1);
                ...
                if (paneGrid.rowIndex === -1) {
                    inc = offset;
                    for (var i = 0, len = heightList.length; i < len; i++, startRow++) {
                        if (heightList[i] === 0) {
                            continue;
                        }
                        viewHeight.push(heightList[i]);
                        inc += heightList[i] + borderWidth;
                        rowMap.push(startRow);
                        rMap[startRow] = rowMap.length - 1;
                        viewRowPoints.push(inc);
                    }
                    grid.rowMap = rowMap;
                    grid.rMap = rMap;
                    ...
                    index = paneGrid.rowIndex + 1;
                    viewHeight = grid.viewHeight.slice(0, index);
                    rowMap = grid.rowMap.slice(0, index);
                    viewRowPoints = grid.viewRowPoints.slice(0, index + 1);
                    inc = viewRowPoints[index];
                    startRow += index;
                    for (var i = 0, len = heightList.length; i < len; i++, startRow++) {
                        if (heightList[i] === 0) {
                            continue;
                ...
                // 使得回归计算后的开始行数在不破坏限制规则的情况下接近于期望的行数
                grid.startRow = this.__approachRow(grid, originalStartRow, grid.startRow);
            ...
            __patchCol: function(grid) {
                ...
                var space = this.dataAccess.getSpace();
                var offset = this.dataAccess.getOffset();
                var totalCol = this.dataAccess.getTotalIndex().col;
                var borderWidth = this.dataAccess.getBorderWidth();
                var widthList;
                if (paneGrid.colIndex !== -1) {
                    widthList = this.__getColPatchInfo(space.width - (grid.viewColPoints[paneGrid.colIndex + 1] + offset) - TAIL_SAPACE, totalCol);
                } else {
                    widthList = this.__getColPatchInfo(space.width - borderWidth - TAIL_SAPACE, totalCol);
                }
                var originalStartCol = grid.startCol;
                grid.startCol = totalCol + 1 - widthList.length - (paneGrid.colIndex + 1);
                ...
                if (paneGrid.colIndex === -1) {
                    inc = offset;
                    for (var i = 0, len = widthList.length; i < len; i++, startCol++) {
                        if (widthList[i] === 0) {
                            continue;
                        }
                        viewWidth.push(widthList[i]);
                        inc += widthList[i] + borderWidth;
                        colMap.push(startCol);
                        cMap[startCol] = colMap.length - 1;
                        viewColPoints.push(inc);
                    }
                    grid.colMap = colMap;
                    grid.cMap = cMap;
                    ...
                    index = paneGrid.colIndex + 1;
                    viewWidth = grid.viewWidth.slice(0, index);
                    colMap = grid.colMap.slice(0, index);
                    viewColPoints = grid.viewColPoints.slice(0, index + 1);
                    inc = viewColPoints[index];
                    startCol += index;
                    for (var i = 0, len = widthList.length; i < len; i++, startCol++) {
                        if (widthList[i] === 0) {
                            continue;
                    ...
                    for (var i = 0, len = colMap.length; i < len; i++) {
                        cMap[colMap[i]] = i;
                    }
                    grid.colMap = colMap;
                    grid.cMap = cMap;
                ...
                grid.startCol = this.__approachColumn(grid, originalStartCol, grid.startCol);
                return grid.startCol;
            },
            __getRowPatchInfo: function(spaceHeight, totalRow) {
                var dataAccess = this.dataAccess;
                var borderWidth = dataAccess.getBorderWidth();
                var cell = this.cell;
                var heightList = [];
                ...
                return heightList.reverse();
            },
            __getColPatchInfo: function(spaceWidth, totalCol) {
                var dataAccess = this.dataAccess;
                var borderWidth = dataAccess.getBorderWidth();
                var cell = this.cell;
                var widthList = [];
                ...
                return widthList.reverse();
            },
            __approachRow: function(grid, expectedRow, realRow) {
                var rMap = grid.rMap;
                if (expectedRow === realRow) {
                    return realRow;
                }
                if (expectedRow > realRow) {
                    while (rMap[realRow] === undefined) {
                        realRow++;
                    }
                    return realRow;
                } else {
            ...
            __approachColumn: function(grid, expectedCol, realCol) {
                var cMap = grid.cMap;
                if (expectedCol === realCol) {
                    return realCol;
                }
                if (expectedCol > realCol) {
                    while (cMap[realCol] === undefined) {
                        realCol++;
                    }
                    return realCol;
                } else {
...
_p[23] = {
    value: function(require) {
        return _p.r(0).create("WriteCommand", {
            base: _p.r(150),
            name: "content write forcewrite batchwrite",
         ...
         */
            execute: function(name, content, row, col) {
                ...
                } else if (name === "forcewrite") {
                    return this.rs("c.write", content, row, col, true);
...
//src/core/data-validation/index.js
/**
 * @file 数据验证
 * @author hancong03@baiud.com
 */
_p[41] = {
    value: function(require) {
        ...
        var DataValidation = _p.r(0).create("DataValidation", {
            ...
            init: function(dataAccess) {
                this.dataAccess = dataAccess;
                this.validator = this.createComponent(Validator);
                this.initService();
            },
            initService: function() {
                this.registerService({
         ...
         * @returns {boolean}
         */
            validate: function(content, row, col) {
                var dataValidation = this.dataAccess.getDataValidation(row, col);
                if (!dataValidation) {
                    return true;
                }
                return this.validator.validate(content, dataValidation);
            },
...
//src/core/data-validation/validator.js
/**
 * @file 数据验证
 * @author hancong03@baiud.com
 */
_p[42] = {
    value: function(require) {
        return _p.r(0).create("Validator", {
            dataAccess: null,
            cache: {},
            base: _p.r(144),
            validate: function(content, validationOption) {
                var tokens = this.cache[validationOption.key];
                if (this.cache[validationOption.key]) {}
                console.log(validationOption.key);
            }
        });
    }
};
...
_p[44] = {
    value: function(require) {
        ...
        }, _p.r(165), _p.r(50), _p.r(48), _p.r(45), _p.r(47), _p.r(51), _p.r(49), _p.r(46)));
    }
};
...
_p[46] = {
    value: {
        getDataValidation: function(row, col) {
            return this.kernelGetDataValidation(row, col);
        },
        setDataValidation: function(start, end, options) {
...
_p[48] = {
    value: {
        ...
        setFormulaToken: function(row, col, tokens) {
            if (!tokens) {
        ...
        getFormulaTokenInstance: function(row, col) {
            var tokenData = this.kernelGetActiveData().formulaTokenInstance;
            if (tokenData[row] && tokenData[row][col]) {
                return tokenData[row][col];
            }
            return undefined;
        },
        setFormulaTokenInstance: function(row, col, tokens) {
            if (!tokens) {
                this.clearFormulaToken(row, col);
            }
            var tokenData = this.kernelGetActiveData().formulaTokenInstance;
        ...
        clearFormulaTokenInstance: function(row, col) {
            var tokenData = this.kernelGetActiveData().formulaTokenInstance;
            if (tokenData[row] && tokenData[row][col]) {
                tokenData[row][col] = undefined;
            }
...
//src/core/definition/range-type.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
_p[60] = {
    value: function(require) {
        ...
        var CellRelation = {
            getCells: function(tokens, row, col) {
                tokens = CellRelation.getStandardTokens(tokens, row, col);
                var token;
                var currentCells;
                var cells = [];
                // 重复记录
                ...
                for (var i = 0, len = tokens.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = currentCells.length; j < jlen; j++) {
                        cells = cells.concat(getCells(currentCells[j]));
                    }
                }
                // ------- 去重
                var current;
            ...
            getStandardTokens: function(tokens, row, col) {
                var current;
                ...
                for (var i = 0, len = tokens.length; i < len; i++) {
                    ...
                    switch (current.type) {
                      ...
                      case TOKEN_SUBTYPE.UNION:
                        for (var j = 0, jlen = current.value.length; j < jlen; j++) {
                            current.value[j] = standardization(current.value[j], row, col);
                        }
                        break;
        ...
        function getCells(cellUnit) {
            var cells = [];
            var rowRegion = cellUnit.row;
            var colRegion = cellUnit.col;
            if (!rowRegion) {
                for (var i = colRegion.start, limit = colRegion.end; i <= limit; i++) {
                    cells.push({
                        row: undefined,
                        col: i
                    });
            ...
            if (!colRegion) {
                for (var i = rowRegion.start, limit = rowRegion.end; i <= limit; i++) {
                    cells.push({
                        row: i,
                        col: undefined
                    });
            ...
            for (var i = rowRegion.start, limit = rowRegion.end; i <= limit; i++) {
                for (var j = colRegion.start, jlimit = colRegion.end; j <= jlimit; j++) {
                    cells.push({
                        row: i,
                        col: j
                    });
        ...
        function standardization(cells, row, col) {
            __toAbs(cells, row, col);
            cells.row = __getStandardRow(cells.row);
            cells.col = __getStandardColumn(cells.col);
            return cells;
        }
        function __getStandardRow(row) {
            if (!row) {
                return undefined;
            }
            if (row.rel && row.abs) {
                return {
                    start: Math.min(row.rel.start, row.abs.start),
                    end: Math.max(row.rel.end, row.abs.end)
                };
            }
            return row.rel || row.abs;
        }
        function __getStandardColumn(col) {
            if (!col) {
                return undefined;
            }
            if (col.rel && col.abs) {
                return {
                    start: Math.min(col.rel.start, col.abs.start),
                    end: Math.max(col.rel.end, col.abs.end)
                };
            }
            return col.rel || col.abs;
        }
        function __toAbs(cells, row, col) {
            if (cells.row && cells.row.rel) {
                cells.row.rel.start += row;
                cells.row.rel.end += row;
            }
            if (cells.col && cells.col.rel) {
                cells.col.rel.start += col;
                cells.col.rel.end += col;
            }
        }
        return CellRelation;
    }
...
_p[61] = {
    value: function(require) {
        var Parser = _p.r(80);
        var CellRelation = _p.r(60);
        var MathRuntime = _p.r(75);
        var Formula = _p.r(0).create("Formula", {
            ...
            getTokens: function(row, col) {
                var tokens = this.dataAccess.getFormulaTokenInstance(row, col);
                if (tokens) {
                    return tokens;
                }
                var formulaText = this.valueModule.getRawValue(row, col);
                this.setFormula(row, col, formulaText);
                return this.dataAccess.getFormulaTokenInstance(row, col);
            },
            getTokensByText: function(formulaText) {
            ...
            setFormula: function(row, col, formulaText) {
                var tokens = Parser.parse(formulaText, row, col);
                var standardTokens = CellRelation.getStandardTokens(tokens, row, col);
                var cells = CellRelation.getCells(tokens, row, col);
                this.dataAccess.setFormulaToken(row, col, tokens);
                this.dataAccess.setFormulaTokenInstance(row, col, standardTokens);
            ...
            __clearFormula: function(row, col) {
                ...
                this.dataAccess.clearFormulaTokenInstance(row, col);
        ...
        function clearAssociatedFormulaValue(dataAccess, valueModule, row, col) {
            ...
            // column update
            if (columnRelData[col]) {
                columnRelData = columnRelData[col];
...
//src/core/formula/math/array/basic.js
/**
 * @file 基本运算运行时
 * @author hancong03@baiud.com
 */
_p[68] = {
    value: function(require) {
        var ERROR_VALUE = _p.r(124);
        var F_VALUE_TYPE = _p.r(53);
        var OPERAND_TYPE = _p.r(54);
        var ComparisonRuntime = _p.r(70);
        var TypeCast = _p.r(71);
        var FormulaUtils = _p.r(76);
        var RANGE_TYPE = _p.r(55);
        return {
            exec: function(valueModule, operator, operands, start, end) {
                ...
                for (var i = 0, len = operands.length; i < len; i++) {
                    ...
                    // ---- range cells operand
                    value = FormulaUtils.getCellsValue(valueModule, current.value[0], start, end);
                    if (value === null) {
                        return {
                            type: F_VALUE_TYPE.ERROR,
        ...
        function calculate(operator, operands, start, end) {
            ...
            return arrayCalculate(operator, op1, op2);
        }
        function arrayCalculate(operator, op1, op2) {
            var result = [];
            var rowResult;
            var minLength;
            var rangeInfo1 = FormulaUtils.getRangeInfo(op1);
            var rangeInfo2 = FormulaUtils.getRangeInfo(op2);
            if (rangeInfo1.type === RANGE_TYPE.SINGLE) {
                op1 = FormulaUtils.fillAllArray(op1, rangeInfo2.row, rangeInfo2.col);
            } else if (rangeInfo2.type === RANGE_TYPE.SINGLE) {
                op2 = FormulaUtils.fillAllArray(op2, rangeInfo1.row, rangeInfo1.col);
            } else {
                if (rangeInfo1.type === RANGE_TYPE.COLUMN) {
                    op1 = FormulaUtils.fillColumnArray(op1, rangeInfo2.col);
                } else if (rangeInfo1.type === RANGE_TYPE.ROW) {
                    op1 = FormulaUtils.fillRowArray(op1, rangeInfo2.row);
                }
                if (rangeInfo2.type === RANGE_TYPE.COLUMN) {
                    op2 = FormulaUtils.fillColumnArray(op2, rangeInfo1.col);
                } else if (rangeInfo2.type === RANGE_TYPE.ROW) {
                    op2 = FormulaUtils.fillRowArray(op2, rangeInfo1.row);
                }
            }
...
_p[69] = {
    value: function(require) {
            ...
            analyze: function(valueModule, operands, start, end) {
                ...
                if (operand.type === OPERAND_TYPE.UNION) {
                    ...
                    operand = {
                        ...
                        value: FormulaUtils.getCellsValue(valueModule, operand.value[0], start, end)
                    };
                }
        ...
        function toArray(values, start, end) {
            ...
            var colCount = end.col - start.col + 1;
            var rowValues;
            var defaultValue = {
                type: F_VALUE_TYPE.ERROR,
            ...
            for (var i = 0, len = values.length; i < len; i++) {
                rowValues = values[i];
                for (var j = 0, jlen = rowValues.length; j < jlen; j++) {
                    if (rowValues[j].type === F_VALUE_TYPE.UNDEFINED) {
                        rowValues[j] = emptyValue;
                    }
                }
            }
            // 填充
            var rangeInfo = FormulaUtils.getRangeInfo(values);
            switch (rangeInfo.type) {
              case RANGE_TYPE.SINGLE:
                result = FormulaUtils.fillAllArray(values, rowCount, colCount);
                break;

              case RANGE_TYPE.ROW:
                result = fillRow(values, rowCount, colCount, defaultValue);
                break;

              case RANGE_TYPE.COLUMN:
                result = fillColumn(values, rowCount, colCount, defaultValue);
                break;

              case RANGE_TYPE.RANGE:
                result = fillRange(values, rowCount, colCount, defaultValue);
                break;

              default:            }
            return $._clone(result);
        }
        function fillRow(values, rowCount, colCount, defaultValue) {
        ...
        function fillColumn(values, rowCount, colCount, defaultValue) {
            var result = [];
            var rowResult;
            var current;
            for (var i = 0; i < rowCount; i++) {
        ...
        function fillRange(values, rowCount, colCount, defaultValue) {
            var result = [];
            var rowResult;
            var current;
            for (var i = 0; i < rowCount; i++) {
                ...
                for (var j = 0; j < colCount; j++) {
                    current = values[i];
                    if (current) {
                        current = current[j] || defaultValue;
                    } else {
                        current = defaultValue;
                    }
                    rowResult.push(current);
...
//src/core/formula/math/common/comparison.js
/**
...
//src/core/formula/math/common/type-caster.js
/**
 * @file 基本运算运行时
 * @author hancong03@baiud.com
 */
_p[71] = {
    value: function(require) {
        var CELL_TYPE = _p.r(123);
        var LOGICAL_VALUE = _p.r(126);
        return {
...
_p[72] = {
    value: function(require) {
        var ERROR_VALUE = _p.r(124);
        var F_VALUE_TYPE = _p.r(53);
        var FunctionManager = _p.r(67);
        var ValueReader = _p.r(83);
        return {
...
//src/core/formula/math/normal/basic.js
/**
 * @file 基本运算运行时
 * @author hancong03@baiud.com
 */
_p[73] = {
    value: function(require) {
        ...
        return {
            exec: function(valueModule, operator, operands, start, end) {
                ...
                for (var i = 0, len = operands.length; i < len; i++) {
                    ...
                    // ---- range cells operand
                    value = FormulaUtils.getCellsValue(valueModule, value, start, end);
                    // 多值
                    if (FormulaUtils.getRangeInfo(value).type !== RANGE_TYPE.SINGLE) {
                        return {
                            type: F_VALUE_TYPE.ERROR,
                            value: ERROR_VALUE.VALUE
                        };
                    }
                    operands[i] = value[0][0];
                }
                return calculate(operator.value, operands);
            }
        };
        function calculate(operator, operands) {
            ...
            return cal(operator, operands[0], operands[1]);
        }
        function cal(operator, op1, op2) {
            if (op1.type === OPERAND_TYPE.ERROR) {
                return op1;
...
//src/core/formula/math/normal/result-analyzer.js
/**
 * @file
 * @author hancong03@baiud.com
 */
        ...
        return {
            analyze: function(valueModule, operands, start, end) {
                ...
                if (operand.type === OPERAND_TYPE.RANGE) {
                    ...
                    // 无限值
                    if (operand.row == undefined || operand.col === undefined) {
                        return {
                            type: F_VALUE_TYPE.ERROR,
                            value: ERROR_VALUE.VALUE
                        };
                    ...
                    operand = {
                        type: F_VALUE_TYPE.ARRAY,
                        value: FormulaUtils.getCellsValue(valueModule, operand, start, end)
                    };
                }
                if (operand.type !== OPERAND_TYPE.ARRAY) {
                    return operand;
                }
                if (FormulaUtils.getRangeInfo(operand.value).type !== RANGE_TYPE.SINGLE) {
                    return {
                        type: F_VALUE_TYPE.ERROR,
                        value: ERROR_VALUE.VALUE
                    };
                ...
                if (operand.type === F_VALUE_TYPE.UNDEFINED) {
                    return {
                        type: F_VALUE_TYPE.NUMBER,
                        value: "0"
                    };
...
_p[75] = {
    value: function(require) {
        var TOKEN = _p.r(56);
        var TOKEN_TYPE = TOKEN.TYPE;
        var FunctionRuntime = _p.r(72);
        var ArrayBasicRuntime = _p.r(68);
        var NormalBasicRuntime = _p.r(73);
        var NormalResultAnalyzer = _p.r(74);
        var ArrayResultAnalyzer = _p.r(69);
        return _p.r(0).create("MathRuntime", {
            ...
            exec: function(tokens) {
                ...
                for (var i = 0, len = tokens.length; i < len; i++) {
                    ...
                    switch (token.type) {
                      ...
                      case TOKEN_TYPE.OP_IN:
                        stack.push({
                            type: TOKEN_TYPE.OPERAND,
                            value: NormalBasicRuntime.exec(this.valueModule, token, stack.splice(stack.length - 2, 2), start, end)
                        });
                        break;
                      ...
                      case TOKEN_TYPE.OP_POST:
                        stack.push({
                            type: TOKEN_TYPE.OPERAND,
                            value: NormalBasicRuntime.exec(this.valueModule, token, stack.splice(stack.length - 1, 1), start, end)
                        });
                        break;
                ...
                // 分析最终结果
                return NormalResultAnalyzer.analyze(this.valueModule, stack, start, end);
            },
...
_p[76] = {
    value: function(require) {
        var RANGE_TYPE = _p.r(55);
        return {
            getRangeInfo: function(cells) {
                var rowCount = cells.length;
                var columnCount = 0;
                var type;
                for (var i = 0, len = cells.length; i < len; i++) {
                    columnCount = Math.max(columnCount, cells[i].length);
                }
                if (rowCount === 1) {
                    if (columnCount === 1) {
                        type = RANGE_TYPE.SINGLE;
                    } else {
                        type = RANGE_TYPE.ROW;
                    }
                } else {
         ...
         * @param end
         */
            fillAllArray: function(values, rowCount, colCount) {
                var result = [];
                var rowResult;
            ...
            // 填充行
            fillRowArray: function(values, rowCount) {
                var result = [];
                values = values[0];
                for (var i = 0; i < rowCount; i++) {
                    result.push(values);
            ...
            fillColumnArray: function(values, colCount) {
                var result = [];
                var rowResult;
                var current;
                for (var i = 0, len = values.length; i < len; i++) {
        ...
        function putValue(valueModule, result, start, end) {
            var rowResult;
            for (var i = start.row, limit = end.row; i <= limit; i++) {
                rowResult = [];
                for (var j = start.col, jlimit = end.col; j <= jlimit; j++) {
                    rowResult.push(valueModule.getComputedValueInfo(i, j));
                }
                result.push(rowResult);
            }
        }
...
//src/core/formula/parser/cell-operation.js
/**
 * @file 计算单元格引用运算符
 * 运算符有： ":"，",", " "(空格)
...
_p[77] = {
     ...
     */
        function getIntersectCells(firstCell, secondCell) {
            ...
            var resultCells = [];
            var tmp;
            for (var i = 0, len = cells1.length; i < len; i++) {
                for (var j = 0, jlen = cells2.length; j < jlen; j++) {
                    tmp = __getIntersect(cells1[i], cells2[j]);
                    if (tmp) {
            ...
            if (resultCells.length === 0) {
                return {
                    type: TOKEN_TYPE.OPERAND,
                    subtype: TOKEN_SUBTYPE.ERROR,
                    cells: ERROR_VALUE.NULL
                };
            }
            if (firstCell.subtype === TOKEN_SUBTYPE.UNION && secondCell.subtype === TOKEN_SUBTYPE.UNION) {
                return {
                    type: TOKEN_TYPE.OPERAND,
            ...
            return {
                type: TOKEN_TYPE.OPERAND,
                subtype: resultCells.length === 1 ? TOKEN_SUBTYPE.RANGE : TOKEN_SUBTYPE.UNION,
                cells: resultCells
            };
        }
        function __getIntersect(cell1, cell2) {
            var row = __getIntersectRow(cell1.row, cell2.row);
            if (row === null) {
                return null;
            }
            var col = __getIntersectColumn(cell1.col, cell2.col);
            if (col === null) {
                return null;
            }
            return {
        ...
        function __getIntersectRow(row1, row2) {
            var start;
            var end;
            if (!row1) {
                return row2;
            ...
            if (!row1.abs && !row2.abs) {
                start = Math.max(row1.rel.start, row2.rel.start);
                end = Math.min(row1.rel.end, row2.rel.end);
                if (start > end) {
                    return null;
                }
                return {
            ...
            if (!row1.rel && !row2.rel) {
                start = Math.max(row1.abs.start, row2.abs.start);
                end = Math.min(row1.abs.end, row2.abs.end);
                if (start > end) {
                    return null;
                }
                return {
            ...
            return {
                abs: row1.abs ? row1.abs : row2.abs,
                rel: row1.rel ? row1.rel : row2.rel
            };
        }
        function __getIntersectColumn(col1, col2) {
            var start;
            var end;
            if (!col1) {
                return col2;
            ...
            if (!col1.abs && !col2.abs) {
                start = Math.max(col1.rel.start, col2.rel.start);
                end = Math.min(col1.rel.end, col2.rel.end);
                if (start > end) {
                    return null;
                }
                return {
            ...
            if (!col1.rel && !col2.rel) {
                start = Math.max(col1.abs.start, col2.abs.start);
                end = Math.min(col1.abs.end, col2.abs.end);
                if (start > end) {
                    return null;
                }
                return {
            ...
            return {
                abs: col1.abs ? col1.abs : col2.abs,
                rel: col1.rel ? col1.rel : col2.rel
            };
        }
        /**
     * 计算联合（并集）单元格
     */
        function getUnionCells(firstCell, secondCell) {
            var result = firstCell.cells;
            result = result.concat(secondCell.cells);
            return {
                type: TOKEN_TYPE.OPERAND,
                subtype: TOKEN_SUBTYPE.UNION,
                cells: result
...
_p[78] = {
        ...
        return {
            parse: function(tokens, row, col) {
                ...
                for (var i = 0, len = result.length; i < len; i++) {
                    ...
                    current = current.cells[0];
                    if (current.row && current.row.rel) {
                        current.row.rel.start -= row;
                        current.row.rel.end -= row;
                    }
                    if (current.col && current.col.rel) {
                        current.col.rel.start -= col;
                        current.col.rel.end -= col;
                    }
                }
                return result;
            }
     ...
     */
        ...
        function processArray(tokens, result) {
            var arrayResult = [];
            var currentRow;
            var token;
            while (tokens.moveNext()) {
                ...
                switch (token.type) {
                  ...
                  case TOKEN_TYPE.ARRAYROW:
                    if (token.subtype === TOKEN_SUBTYPE.START) {
                        currentRow = [];
                        arrayResult.push(currentRow);
                    }
                    break;
        ...
        function getCells(text) {
            ...
            var tmp;
            if (groups.length === 1) {
                cell = getSingleCellInfo(getCellInfo(groups[0]));
            } else {
                tmp = [];
                for (var i = 0, len = groups.length; i < len; i++) {
                    tmp[i] = getCellInfo(groups[i]);
                }
                cell = getRegionCellInfo(tmp);
            }
            return [ cell ];
        }
        ...
        function getSingleCellInfo(cell) {
            var region = {
                row: {},
                col: {}
            };
        ...
        function getRegionCellInfo(cells) {
            var current;
            var region = {
                row: {},
                col: {}
            ...
            for (var i = 0, len = cells.length; i < len; i++) {
                ...
                if (region.row) {
                    if (current.row.abs) {
                        absRow.push(current.row.value);
                    } else {
                        relRow.push(current.row.value);
                    }
                }
                if (region.col) {
                    if (current.col.abs) {
                        absCol.push(current.col.value);
                    } else {
                        relCol.push(current.col.value);
                    }
                }
            }
            if (region.row) {
                if (absRow.length === 0) {
                    region.row.abs = undefined;
                } else {
                    region.row.abs = {
                        start: Math.min.apply(null, absRow),
                        end: Math.max.apply(null, absRow)
                    };
                }
                if (relRow.length === 0) {
                    region.row.rel = undefined;
                } else {
                    region.row.rel = {
                        start: Math.min.apply(null, relRow),
                        end: Math.max.apply(null, relRow)
                    };
                }
            }
            if (region.col) {
                if (absCol.length === 0) {
                    region.col.abs = undefined;
                } else {
                    region.col.abs = {
                        start: Math.min.apply(null, absCol),
                        end: Math.max.apply(null, absCol)
                    };
                }
                if (relCol.length === 0) {
                    region.col.rel = undefined;
                } else {
                    region.col.rel = {
                        start: Math.min.apply(null, relCol),
                        end: Math.max.apply(null, relCol)
                    };
                }
            }
            return region;
...
_p[80] = {
    value: function(require) {
        var TokenAnalyzer = _p.r(82);
        var CellsParser = _p.r(78);
        var RPNParser = _p.r(81);
        var CellOperation = _p.r(77);
        var OperandParser = _p.r(79);
        return {
            parse: function(text, row, col) {
                var tokens = TokenAnalyzer.getTokens(text);
                tokens = CellsParser.parse(tokens, row, col);
                tokens = RPNParser.parse(tokens);
                tokens = CellOperation.calculate(tokens);
...
_p[98] = {
    value: function(require) {
        var CELL_TYPE = _p.r(123);
        var NumberFormatter = _p.r(113);
        var LOGICAL_VALUE = _p.r(126);
        var CodeParser = _p.r(97);
...
_p[100] = {
    value: function(require) {
        var CELL_TYPE = _p.r(123);
        var NumberAnalyzer = _p.r(103);
        var LogicalAnalyzer = _p.r(102);
        var ErrorAnalyzer = _p.r(101);
        return {
            exec: function(content, numberFormatCode) {
                var result;
                if (!content) {
...
_p[103] = {
    value: function(require) {
        var NumericAnalyzer = _p.r(107);
        var DateTimeAnalyzer = _p.r(105);
        var CurrencyAnalyzer = _p.r(104);
        var PercentAnalyzer = _p.r(108);
        var FractionAnalyzer = _p.r(106);
        var CodeParser = _p.r(97);
        var CODE_FLAG = _p.r(96);
        return {
...
_p[105] = {
    value: function(require) {
        var CELL_TYPE = _p.r(123);
        var NUMBER_FORMAT = _p.r(128);
        var DateTimeConverter = _p.r(109);
        var FORMAT_CODE = _p.r(125);
        var MAX_YEAR = 9999;
        var MIN_YEAR = 1900;
        return {
...
_p[113] = {
    value: function(require) {
        var CodeParser = _p.r(97);
        var DecimalFormatter = _p.r(115);
        var ScientificFormatter = _p.r(117);
        var DateTimeFormatter = _p.r(114);
        var FractionFormatter = _p.r(116);
        var CODE_FLAG = _p.r(96);
        var FORMAT_CODE = _p.r(125);
        return {
...
_p[119] = {
    value: function(require) {
        ...
        var NUMBER_FORMAT = _p.r(128);
        var Format = _p.r(110);
        var Formatter = _p.r(112);
        var ColorManager = _p.r(98);
        var Value = _p.r(0).create("Value", {
            dataAccess: null,
            formulaModule: null,
            rangeFacade: null,
            ...
            colorManager: null,
            base: _p.r(147),
            init: function(dataAccess, formulaModule, valueSecurityModule, validationModule) {
                this.dataAccess = dataAccess;
            ...
            writeCell: function(content, row, col, isCSE) {
                ...
                if (this.validationModule.validate(content, row, col)) {}
                this.corePrepareRefresh();
        ...
        Value.deps = [ "dataAccess", "formula", "valueSecurity", "dataValidation" ];
        return Value;
    }
};
    ...
    value: function(require) {
         ...
         */
            ...
            __resetRowToStart: function(row) {
                ...
                //if (row < startRow && grid.rowInStart) {
                //    return;
                //}
                //
                //if (row > startRow && grid.rowInEnd) {
                //    return;
                //}
            ...
            __resetColumnToStart: function(col) {
                ...
                //
                //if (col < startCol && grid.colInStart) {
                //    return;
                //}
                //
                //if (col > startCol && grid.colInEnd) {
                //    return;
                //}
...
_p[132] = {
    value: function(require) {
        var Container = _p.r(135);
        var ElementFactory = _p.r(136);
        var Service = _p.r(139);
        var Command = _p.r(134);
        var Message = _p.r(137);
        var RefreshManager = _p.r(138);
        var Configure = _p.r(131);
        var ModuleLoader = _p.r(140);
        var PluginWrapper = _p.r(141);
...
_p[154] = {
    value: function(require) {
        ...
        var STYLE_GROUP = _p.r(189).styleGroup;
        var DEFAULT_STYLE_VALUE = _p.r(158);
        var FORMAT_CODE = _p.r(125);
        var VALUE_FILTER = _p.r(153);
        var FORMAT_CODE_MAP = {};
        for (var key in FORMAT_CODE) {
            if (FORMAT_CODE.hasOwnProperty(key) && key !== "_default") {
                FORMAT_CODE_MAP[FORMAT_CODE[key]] = key;
...
_p[155] = {
    value: function(require) {
        ...
        function __importSignleSheet(currentSheet) {
            ...
            if (currentSheet.sheetView.showRowColHeaders === 0) {
                res.header = false;
            }
            if (currentSheet.sheetView.showGridLines === 0) {
                res.showGrid = false;
            }
            res.row = rowInfo.height;
            res.rowStyle = rowInfo.style;
...
_p[162] = {
    value: function(require) {
        var MAX_COLUMN_COUNT = _p.r(185).MAX_COLUMN_COUNT;
        return {
...
_p[165] = {
    value: function(require) {
        ...
        }, _p.r(162), _p.r(182), _p.r(178), _p.r(173), _p.r(183), _p.r(180), _p.r(179), _p.r(184), _p.r(176), _p.r(166), _p.r(174), _p.r(175), _p.r(180), _p.r(166), _p.r(171), _p.r(167), _p.r(170), _p.r(169), _p.r(172), _p.r(177), _p.r(168));
    }
};
...
_p[168] = {
    value: function(require) {
        var DEFAULT_VALIDATION = _p.r(160);
        return {
            kernelGetDataValidation: function(row, col) {
                var activeData = this.kernelGetActiveData();
                if (!activeData.dataValidationRel[row] || activeData.dataValidationRel[row][col] === undefined) {
                    return undefined;
                }
                var index = activeData.dataValidationRel[row][col];
                return activeData.dataValidation[index];
            },
            kernelSetDataValidation: function(start, end, options) {
                options = $.extend(true, {}, DEFAULT_VALIDATION, options);
                var activeData = this.kernelGetActiveData();
                var key = JSON.stringify(options);
                var index = activeData.dataValidationRecord[key];
                if (index === undefined) {
                    ...
                    activeData.dataValidationRecord[key] = index;
                }
                var rels = activeData.dataValidationRel;
                for (var i = start.row, limit = end.row; i <= limit; i++) {
                    if (!rels[i]) {
                        rels[i] = [];
                    }
...
_p[173] = {
    value: function(require) {
         ...
         */
            ...
            kernelImportWorkbook: function(data) {
                ...
                if (data) {
                    ...
                    data = this.kernelCreateWorkbook();
                }
        ...
        function importHidden(data) {
            var sheets = data.sheets;
            for (var i = 0, len = sheets.length; i < len; i++) {
                sheets[i].hidden = indexMapToArray(sheets[i].hidden);
            }
        }
        function indexMapToArray(hiddenData) {
            var result = [];
            for (var key in hiddenData) {
                if (hiddenData.hasOwnProperty(key)) {
                    result[key] = hiddenData[key];
                }
            }
            return result;
        }
...
_p[180] = {
    value: function(require) {
        var StyleUtil = _p.r(181);
        var APPLY_CLASSIFY = _p.r(187).applyName;
        return {
...
_p[184] = {
    value: function(require) {
        ...
        var XF_STYLE_DEFAULT = _p.r(186).xfStyle;
        return {
    ...
    value: function(require) {
        return {
            ...
            core: {
                ...
                visible: _p.r(121),
                device: _p.r(57),
                exportManager: _p.r(58),
                wraptext: _p.r(122),
                autosize: _p.r(16),
                formula: _p.r(61),
                valueSecurity: _p.r(95),
                mode: _p.r(86),
                comments: _p.r(39),
                paster: _p.r(87),
                header: _p.r(84),
...
_p[196] = {
    value: function(require) {
        var HTMLConverter = _p.r(197);
        var PlaintextConverter = _p.r(200);
        var OutConverter = _p.r(199);
...
_p[197] = {
    value: function(require) {
        var ELEMENT = 1;
        var TEXT = 3;
        var ROW_ELEMENTS = _p.r(201);
        var IGNORE_ELEMENTS = _p.r(198);
        return _p.r(0).create("Converter", {
            shadowNode: null,
            base: _p.r(144),
            init: function() {
    ...
    value: {
        keyEnter: function(evt) {
            ...
            if (!this.rs("c.is.readonly")) {
                ...
                // 退出编辑状态
                this.write((evt.metaKey || evt.ctrlKey) && evt.shiftKey);
                this.toInactive();
            }
            if (!evt.shiftKey) {
...
_p[212] = {
    value: function(require) {
        ...
        return _p.r(0).create("InputWorker", $.extend({
            ...
            write: function(isForce) {
                if (!this.contentchanged) {
                    return false;
                }
                this.contentchanged = false;
                var content = this.getContent();
                this.execCommand(!isForce ? "write" : "forcewrite", content, this.activeCell.row, this.activeCell.col);
            },
        ...
        }, _p.r(211), _p.r(207), _p.r(208), _p.r(210)));
    }
};
    ...
    value: function(require) {
        ...
        function drawBorders(canvas, struct, borders, overflowRecord) {
            ...
            for (var i = 0, len = borders.length; i < len; i++) {
                ...
                if (mergeInfo) {
                    drawMergeCellBorder(canvas, struct, current, mergeInfo, mergeRecord);
                } else {
        ...
        function drawBorder(canvas, struct, borderIndex, rowIndex, colIndex, overflowRecord) {
            ...
            if (borderOption.top !== NONE) {
                x = grid.viewColPoints[colIndex];
                y = grid.viewRowPoints[rowIndex];
                topIsMedium = BORDER_TYPE_WIDTH[borderOption.top.style] === 2;
            ...
            if (borderOption.left !== NONE && !overflowRecord[rowIndex + "," + colIndex]) {
                ...
                if (!topIsMedium) {
                    ...
                    __drawVBorder(canvas, borderOption.left, x, y - BORDER_WIDTH, height + BORDER_WIDTH);
                }
            }
        }
        function drawMergeCellBorder(canvas, struct, current, mergeInfo, mergeRecord) {
            var r = current.r;
            var c = current.c;
            var row = current.row;
            var col = current.col;
            ...
            var key = mergeStartRow + "," + mergeStartCol;
            if (!mergeRecord[key]) {
                mergeRecord[key] = false;
            }
            ...
            if (row === mergeStartRow && borderOption.top !== NONE) {
                ...
                y = grid.viewRowPoints[r];
                mergeRecord[key] = BORDER_TYPE_WIDTH[borderOption.top.style] === 2;
                __drawHBorder(canvas, borderOption.top, x, y, width);
            }
        ...
        // 横向边框绘制和纵向边框绘制在对齐上有细微差别
        function __drawHBorder(canvas, borderStyle, x, y, width) {
            x -= BORDER_OFFSET;
            var lineWidth = BORDER_TYPE_WIDTH[borderStyle.style];
            var space = BORDER_TYPE_SPACE[borderStyle.style];
            var offset = lineWidth === 2 ? BORDER_OFFSET : 0;
            y -= offset;
    ...
    value: function(require) {
        ...
        return {
            draw: function(canvas, struct) {
                ...
                // boundary bottom
                canvas.moveTo(0, boundary.height - offset);
                canvas.lineTo(boundary.width, boundary.height - offset);
                // right
                canvas.moveTo(space.width - offset, 0);
                canvas.lineTo(space.width - offset, space.height);
                // boundary right
                canvas.moveTo(boundary.width - offset, 0);
                canvas.lineTo(boundary.width - offset, boundary.height);
                canvas.stroke();
                canvas.restore();
            }
        };
...
_p[221] = {
    value: function(require) {
        ...
        var PANE_LINE_WIDTH = SYS_CONFIG.PANE_LINE_WIDTH;
        return {
            draw: function(canvas, struct) {
                var pane = struct.getPane();
                ...
                var y;
                canvas.save();
                canvas.lineWidth(PANE_LINE_WIDTH);
                canvas.beginPath();
...
_p[222] = {
    value: function(require) {
        var ContentFillPainter = _p.r(225);
        var ContentPainter = _p.r(226);
        var MergeFillPainter = _p.r(227);
        var CellFillPainter = _p.r(223);
        var Commentspainter = _p.r(224);
        var GridPainter = _p.r(220);
        var BorderPainter = _p.r(218);
        var BoxPainter = _p.r(219);
        var PaneLinePainter = _p.r(221);
        return {
    ...
    value: function(require) {
        ...
        function render(canvas, struct, region) {
            ...
            for (var i = 0, len = fillLayout.length; i < len; i++) {
                fillCell(canvas, fillLayout[i]);
            }
        }
        function fillCell(canvas, current) {
            canvas.fillStyle(current.fill);
        ...
        function getFillLayout(struct, region) {
            ...
            for (var i = region.startRow, limit = region.endRow; i <= limit; i++) {
                ...
                for (var j = region.startCol, jlimit = region.endCol; j <= jlimit; j++) {
                    ...
                    fillStyle = struct.getUserStyle("fills", rowIndex, colIndex);
    ...
    value: function(require) {
        ...
        return {
            draw: function(canvas, struct, contentLayout) {
                ...
                canvas.save();
                canvas.beginPath();
                canvas.rect(0, 0, boundary.width, boundary.height);
                canvas.clip();
        ...
        function render(canvas, layout) {
            ...
            for (var key in layout) {
                ...
                for (var i = 0, len = currentLayout.length; i < len; i++) {
                    ...
                    // 合并后的单元格交由MegeFillPainer处理
                    if (cellLayout.mergeInfo) {
                        continue;
                    } else {
...
_p[226] = {
    value: function(require) {
                ...
                for (var j = start.c, jlimit = end.c; j <= jlimit; j++) {
                    ...
                    if (mergeInfo) {
                        ...
                        if (!mergeRecord[mergeKey]) {
                            ...
                            if (!isVisiableMergeCell(struct, mergeInfo)) {
                                continue;
                            }
                        } else {
                            // 已读取该合并后的单元格
                        ...
                        if (rMap[row] === undefined || cMap[col] === undefined) {
                            continue;
                        }
                        mergeStartRow = row;
                        mergeStartCol = col;
        ...
        function isVisiableMergeCell(struct, mergeInfo) {
            var grid = struct.getGrid();
            var rMap = grid.rMap;
            var cMap = grid.cMap;
            var hasRow = false;
            for (var i = mergeInfo.start.row, limit = mergeInfo.end.row; i <= limit; i++) {
                if (rMap[i] !== undefined) {
                    hasRow = true;
                    break;
                }
            ...
            for (var i = mergeInfo.start.col, limit = mergeInfo.end.col; i <= limit; i++) {
                if (cMap[i] !== undefined) {
                    return true;
                }
            }
    ...
    value: function(require) {
        ...
        function render(canvas, struct, region) {
            ...
            for (var i = 0, len = mergeInfo.length; i < len; i++) {
                ...
                // 内部内容区域填充
                if (currentInfo.fillColor === NONE) {
                    canvas.fillStyle(CANVAS_COLOR);
                } else {
                    canvas.fillStyle(currentInfo.fillColor);
                }
                canvas.fillRect(currentInfo.x + LINE_WIDTH, currentInfo.y + LINE_WIDTH, currentInfo.width - 2 * LINE_WIDTH, currentInfo.height - 2 * LINE_WIDTH);
            }
        }
        function getMergeCellInfo(struct, region) {
            ...
            for (var i = region.startRow, limit = region.endRow; i <= limit; i++) {
                ...
                for (var j = region.startCol, jlimit = region.endCol; j <= jlimit; j++) {
                    ...
                    mergeRecord[mergeKey] = true;
                    rect = Util.getMergeCellVisibleRect(struct, mergeInfo);
                    if (!rect) {
                        continue;
                    }
...
_p[230] = {
    value: function(require) {
        var BodyScreen = _p.r(216);
        var TopScreen = _p.r(231);
        var LeftScreen = _p.r(229);
...
_p[232] = {
    value: function(require) {
        ...
        var LINE_WIDTH = SYS_CONFIG.border.width;
        var LINE_OFFSET = SYS_CONFIG.border.offset;
        var InputUtil = _p.r(209);
        return {
            getMergeCellVisibleRect: function(struct, mergeInfo) {
                return InputUtil.getMergeCellVisibleRect(struct, mergeInfo);
            },
...
_p[234] = {
    value: function(require) {
        ...
        var ScrollEventManager = _p.r(233);
...
_p[238] = {
    value: function(require) {
        var $ = _p.r(2);
        var Util = _p.r(209);
        var NormalWorker = _p.r(236);
        var PaneWorker = _p.r(237);
...
_p[239] = {
    value: function(require) {
        var $ = _p.r(2);
        var CELL_VALUE_TYPE = _p.r(123);
        var BOOLEAN_VALUE = _p.r(126);
        return _p.r(0).create("Cell", {
            base: _p.r(147),
            init: function() {
                this.initService();
...
_p[241] = {
    value: function(require) {
        ...
        BTable.module(_p.r(190));
        BTable.mapEvent(_p.r(142));
        BTable.ExcelAdapter = _p.r(151);
