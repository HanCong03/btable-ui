...
_p[101] = {
    value: function(require) {
        ...
        var STYLE_GROUP = _p.r(129).styleGroup;
...
_p[118] = {
    value: function(require) {
        ...
        }, _p.r(106), _p.r(121), _p.r(124), _p.r(119), _p.r(122), _p.r(123), _p.r(124), _p.r(120), _p.r(119));
    }
};
         ...
         */
            kernelExport: function() {
                ...
                return JSON.stringify({
                    ...
                    rowStyle: toMap(data.rowStyle),
                    columnStyle: toMap(data.columnStyle),
            ...
            kernelImport: function(data) {
                ...
                data.rowStyle = mapToArray(data.rowStyle);
                data.columnStyle = mapToArray(data.columnStyle);
                return data;
            }
        };
...
_p[124] = {
    value: function(require) {
        var StyleUtil = _p.r(125);
        var APPLY_CLASSIFY = _p.r(127).applyName;
        return {
            ...
            __kernelGetStyle: function(classify, row, col) {
                ...
                var styleIndex = data.styleIndex;
                var index = cellStyle[row][col];
                if (index !== undefined && styleIndex[index][applyName] === 1) {
                    return styleIndex[index][classify];
                }
                index = rowStyle[row];
                if (index !== undefined && styleIndex[index][applyName] === 1) {
                    return styleIndex[index][classify];
    ...
    value: function(require) {
        ...
        return {
            draw: function(canvas, contentCanvas, struct) {
                var data = struct.dataAccess.data;
                var borders = struct.getBorders(struct.getViewStart(), struct.getViewEnd());
                canvas.save();
                drawBorders(canvas, contentCanvas, struct, data, borders);
                canvas.restore();
                // clear cache
                CACHE = {};
            }
        };
        function drawBorders(canvas, contentCanvas, struct, data, borders) {
            var rowBorders;
            var current;
            for (var i = 0, len = borders.length; i < len; i++) {
                ...
                for (var j = 0, jlen = rowBorders.length; j <= jlen; j++) {
                    ...
                    drawBorder(canvas, contentCanvas, struct, data, current, i, j);
                }
            }
        }
        function drawBorder(canvas, contentCanvas, struct, data, borderIndex, rowIndex, colIndex) {
            var borderOption = data.styleGroup.borders[borderIndex].border;
            var boundary = struct.getBoundary();
            var grid = struct.getGrid();
            var imageData;
    ...
    value: function(require) {
        ...
        return {
            draw: function(canvas, struct) {
                ...
                var data = struct.dataAccess.data;
                var fillLayout = getFillLayout(struct, data);
                var rowLayout;
                for (var i = 0, len = fillLayout.length; i < len; i++) {
                    rowLayout = fillLayout[i];
                    for (var j = 0, jlen = rowLayout.length; j < jlen; j++) {
                        fillCell(canvas, data, rowLayout[j]);
                    }
                }
                canvas.restore();
            }
        ...
        function fillCell(canvas, data, current) {
            var fillGroup = data.styleGroup.fills;
            var fillIndex = data.styleIndex[current.styleIndex].fills;
            var fillColor = fillGroup[fillIndex].fill;
            if (fillColor === NONE) {
                return;
            }
        ...
        function getFillLayout(struct, data) {
            var viewStart = struct.getViewStart();
            var viewEnd = struct.getViewEnd();
            var startRow = viewStart.row;
            var startCol = viewStart.col;
            ...
            for (var i = startRow; i <= endRow; i++, rowIndex++) {
                ...
                for (var j = startCol; j <= endCol; j++, colIndex++) {
                    currentIndex = getFill(i, j, styleIndex, cellStyle, rowStye, columnStyle);
                    if (!currentIndex) {
                        nextIndex = -1;
                        continue;
                    }
        ...
        function getFill(row, col, styleIndex, cellStyle, rowStyle, columnStyle) {
            var current = cellStyle[row][col];
            if (current && styleIndex[current].applyFill === 1) {
                return current;
            }
            current = rowStyle[row];
            if (current && styleIndex[current].applyFill === 1) {
                return current;
            }
            current = columnStyle[col];
            if (current && styleIndex[current].applyFill === 1) {
                return current;
            }
            return undefined;
