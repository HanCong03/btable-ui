    ...
    value: function(require) {
        ...
        function getSegments(code) {
            ...
            var segments = [ {
                ...
                operator: undefined,
                color: undefined,
                flag: CODE_FLAG.DECIMAL
            } ];
            var segmentIndex = 0;
            var currentSegment = segments[segmentIndex].front;
            var beginString = false;
            // 0: int, 1: decimal, 2: exponent
            var contextFlag = 0;
            var bracketValue = "";
            var nextValue;
            var dateChar;
            while (index < length) {
                ...
                switch (character) {
                  ...
                  case "!":
                    if (beginString) {
                        currentSegment.push(character);
                    } else {
                        // 取下一个字符
                        currentSegment.push(code[index]);
                        // 跳过下一个字符
                        index++;
                    }
                    break;
                  ...
                  case "[":
                    if (beginString) {
                        ...
                        while (nextValue = code[index]) {
                            if (nextValue === '"' || nextValue === ";") {
                                // error
                                throw new Error("format code: bracket error");
                            }
                            index++;
                            if (nextValue !== "]") {
                                bracketValue += nextValue;
                            } else {
                                break;
                            }
                        }
                        if (LOGICAL_OPERATOR_PATTERN.test(bracketValue)) {
                            // 逻辑运算符
                            segments[segmentIndex].operator = RegExp["$&"];
                            segments[segmentIndex].value = parseFloat(RegExp["$'"]);
                        } else {
                            // 颜色
                            segments[segmentIndex].color = bracketValue;
                        }
                    }
                    break;
                  ...
                  case '"':
                    if (!beginString) {
                        ...
                        } else if (contextFlag === 1) {
                            currentSegment = currentSegment.back;
                        } else {
                            currentSegment = currentSegment.exponent;
                        }
                    }
                    break;
                  ...
                  case ".":
                    currentSegment.push(".");
                    if (contextFlag === 0 && !beginString) {
                        contextFlag = 1;
                        currentSegment = segments[segmentIndex].back;
                    }
                    break;
                  ...
                  case ";":
                    if (!beginString) {
                        ...
                        contextFlag = 0;
                        currentSegment = segments[segmentIndex].front;
                    }
                    break;
                  ...
                  case "?":
                    ...
                    if (!beginString) {
                        if (contextFlag === 0) {
                            segments[segmentIndex].frontPlaceholder++;
                        } else if (contextFlag === 1) {
                            segments[segmentIndex].backPlaceholder++;
                        } else {
                            segments[segmentIndex].exponentPlaceholder++;
                        }
                    }
                    break;
                  ...
                  case "e":
                    if (!beginString) {
                        segments[segmentIndex].flag |= CODE_FLAG.SCIENTIFIC;
                        // 切入到指数计算环境
                        currentSegment = segments[segmentIndex].exponent;
                        contextFlag = 2;
                    } else {
                        currentSegment.push(character);
                    }
                  ...
                  case "Y":
                    if (beginString) {
                        ...
                        // 标识存在日期code
                        segments[segmentIndex].flag |= CODE_FLAG.DATE;
                        dateChar = character.toLowerCase();
                        while (code[index] && code[index].toLowerCase() === dateChar.charAt(0)) {
                            dateChar += code[index].toLowerCase();
                            index++;
                        }
                        currentSegment.push(dateChar);
                    }
                    break;
                  ...
                  //case 'h':
                    ...
                    //
                    //    if (!beginString) {
                    //        segments[segmentIndex].hasDate = true;
                    //    }
                    //    currentSegment
                    //    break;
                    case "/":
                    ...
                    if (!beginString) {
                        // 标识存在日期code
                        segments[segmentIndex].__fraction = true;
                    }
                    break;
            ...
            for (var i = 0, len = segments.length; i < len; i++) {
                segments[i].front.reverse();
            }
            CODE_CACHE[originalCode] = segments;
            return segments;
        }
    }
};
...
_p[48] = {
    value: function(require) {
        var CELL_TYPE = _p.r(71);
        var NumberFormatter = _p.r(63);
        var LOGICAL_VALUE = _p.r(74);
    ...
    value: function(require) {
        ...
        return {
            ...
            timeToNumber: function(hour, minute, second) {
                return (hour * 24 + minute * 60 + second * 60) / SECOND_OF_DAY;
            },
...
_p[61] = {
    value: function(require) {
        ...
        var LITERAL = "$¥+(:^'{<=-/)&~}> .";
        return {
            formatLeftPart: function(value, segment, segmentIndex) {
        ...
        function __doFrontFormat(value, segment) {
            ...
            var currentCodeChar;
            var firstString = "";
            // 如果code段的“开头”是字符串，则把该字符串提取出来单独处理
            while ($.isArray(segment[segment.length - 1])) {
                firstString += segment[segment.length - 1].join("");
                segment.length -= 1;
            }
            var valueIndex = 0;
            ...
            if (firstString) {
                segment.push([ firstString ]);
            }
            return firstString + value.reverse().join("") + result.reverse().join("");
        }
...
_p[63] = {
    value: function(require) {
        ...
        var ScientificFormatter = _p.r(66);
        var DateFormatter = _p.r(64);
        var CODE_FLAG = _p.r(46);
        return {
            exec: function(value, code) {
                ...
                switch (segmentInfo.segment.flag) {
                  ...
                  case CODE_FLAG.DATE:
                    return DateFormatter.exec(value, segmentInfo);
                }
            },
...
//src/core/value/formatter/number/date.js
/**
 * @file
 * @author hancong03@baiud.com
 */
    ...
    value: function(require) {
        ...
        return {
            exec: function(value, segmentInfo) {
                ...
                return format(value, segmentInfo.segment.front);
            }
        };
        function format(value, codeSegment) {
    ...
    value: function(require) {
        ...
        return {
            exec: function(value, segmentInfo) {
                ...
                value = (+value).toFixed(segment.backPlaceholder);
                value = (value + "").split(".");
                var front = value[0];
                var back = value[1] || "";
                front = FormatUtil.formatLeftPart(front, segment.front);
                back = FormatUtil.formatRightPart(back, segment.back);
                return front + back;
            }
        };
    }
    ...
    value: function(require) {
        ...
        return {
            exec: function(value, segmentInfo) {
                ...
                value = (+value).toExponential(segment.backPlaceholder);
                var match = SPLIT_PATTERN.exec(value);
                var intValue = FormatUtil.formatLeftPart(match[1], segment.front);
                var decimalValue = "";
                if (match[2]) {
                    decimalValue = FormatUtil.formatRightPart(match[2], segment.back);
                }
                var exponentValue = FormatUtil.formatLeftPart(match[4], segment.exponent.slice(1));
                var exponentSign = segment.exponent[0];
                var sign;
                if (match[3] === "-") {
                    sign = "-";
                } else {
                ...
                return intValue + decimalValue + "E" + sign + exponentValue;
            }
        };
    }
};
...
_p[101] = {
    value: function(require) {
        ...
        function importExcel(excelData) {
            ...
            result.border = importBorder(excelData);
            result.rowBorder = importRowBorder(excelData);
            result.columnBorder = importColumnBorder(excelData);
            result.map = map;
            return result;
        }
        ...
        function importColumnBorder(data) {
...
_p[105] = {
    value: function(require) {
        var MAX_COLUMN_COUNT = _p.r(126).MAX_COLUMN_COUNT;
        return {
...
_p[116] = {
    value: function(require) {
        var StyleCommand = _p.r(113);
        var ResizeCommand = _p.r(112);
        var ClearCommand = _p.r(109);
        var WriteCommand = _p.r(114);
        var ExportCommand = _p.r(110);
        var PasteCommand = _p.r(111);
        return {
            write: WriteCommand,
...
_p[117] = {
    value: function(require) {
        ...
        }, _p.r(105), _p.r(121), _p.r(124), _p.r(125), _p.r(118), _p.r(119), _p.r(127), _p.r(120), _p.r(122), _p.r(123));
    }
};
    ...
    value: function(require) {
        ...
        return {
            kernelGetBorder: function(row, col) {
                ...
                for (var i = 0, len = BORDER_TYPE.length; i < len; i++) {
                    ...
                    value = this.__kernelGetCellBorder(borderType, row, col) || this.__kernelGetRowBorder(borderType, row) || this.__kernelGetColumnBorder(borderType, col);
                    if (value && value !== NONE) {
                        result[borderType] = value;
                    } else {
            ...
            kernelSetAllBorder: function(borderOptions) {
                ...
                for (var i = 0, limit = totalIndex.col; i <= limit; i++) {
                    ...
                    for (var borderType in currentBorderOptions) {
                        ...
                        columnBorderData[borderType][i] = currentBorderOptions[borderType] || {};
                    }
                }
            },
            kernelSetRowBorder: function(borderOptions, startIndex, endIndex) {
                ...
                if (borderOptions === undefined || borderOptions === null || borderOptions === NONE) {
                    borderOptions = $._clone(LOCAL_BORDER_CONFIG);
                } else {
            ...
            kernelAddRowBorder: function(borderOptions, startIndex, endIndex) {
                ...
                for (var i = startIndex; i <= endIndex; i++) {
                    ...
                    for (var borderType in currentBorderOptions) {
                        ...
                        rowBorderData[borderType][i] = currentBorderOptions[borderType];
                    }
                }
                var index;
                /* ------- 清除上一行的数据 -------*/
                if (startIndex - 1 >= 0) {
                    // 行清空
                    rowBorderData["bottom"][startIndex - 1] = undefined;
                    // 单元格清空
                    index = HASH.indexToKey(startIndex - 1, 0);
                    for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                        borderData["bottom"][index] = undefined;
                    }
                }
                /* ------- 清除下一行的数据 -------*/
                // 行清空
                rowBorderData["top"][endIndex + 1] = undefined;
                // 单元格清空
            ...
            kernelAddColumnBorder: function(borderOptions, startIndex, endIndex) {
                ...
                if (currentColumnIndex >= 0) {
                    // 列清空
                    columnBorderData["right"][currentColumnIndex] = undefined;
                    // 单元格清空
                    for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                        index = HASH.indexToKey(0, currentColumnIndex);
                ...
                // 列清空
                columnBorderData["left"][currentColumnIndex] = undefined;
                // 单元格清空
                for (var i = 0, limit = totalIndex.col; i <= limit; i++, index++) {
                    index = HASH.indexToKey(0, currentColumnIndex);
         ...
         */
                ...
                for (var i = start.row, limit = end.row; i <= limit; i++) {
                    ...
                    for (var j = start.col, jlimit = end.col; j <= jlimit; j++, index++) {
                        ...
                        for (var key in borderOptions) {
                            ...
                            borderData[key][index] = currentBorderOptions[key];
                        }
                    }
                }
                // 清除上一行单元格数据
            ...
            __kernelGetCellBorder: function(borderType, row, col) {
                ...
                switch (borderType) {
                  case "left":
                    return borderData.left[HASH.indexToKey(row, col)] || borderData.right[HASH.indexToKey(row, col - 1)];

                  case "top":
                    return borderData.top[HASH.indexToKey(row, col)] || borderData.bottom[HASH.indexToKey(row - 1, col)];

                  case "right":
                    return borderData.left[HASH.indexToKey(row, col + 1)] || borderData.right[HASH.indexToKey(row, col)];

                  case "bottom":
                    return borderData.top[HASH.indexToKey(row + 1, col)] || borderData.bottom[HASH.indexToKey(row, col)];
                }
            },
            /**
         * 读取指定行，指定边框类型的边框值
        ...
        function overlayRowStyle(data, borderOptions, startIndex, endIndex) {
            ...
            var borderData = data.border;
            var keys = Object.keys(rowBorderData);
            var index;
            for (var i = 0, len = keys.length; i < len; i++) {
                ...
                for (var j = startIndex; j <= endIndex; j++, index++) {
                    for (var borderType in borderOptions) {
                        ...
                        borderData[borderType][index] = $._clone(borderOptions[borderType]);
                    }
                }
            }
        }
        function completeBorder(borderData) {
            ...
            for (var i = 0, len = BORDER_TYPE.length; i < len; i++) {
                borderType = BORDER_TYPE[i];
                result[borderType] = borderData[borderType] || LOCAL_BORDER_CONFIG[borderType];
            }
...
_p[123] = {
    value: function(require) {
        var FORMAT_CODE = _p.r(73);
        var FORAMT_CODE_INDEX_MAP = {};
        for (var key in FORMAT_CODE) {
            if (FORMAT_CODE.hasOwnProperty(key) && key !== "_default") {
                FORAMT_CODE_INDEX_MAP[FORMAT_CODE[key]] = key;
            }
        }
        return {
            ...
            kernelSetNumberFormat: function(code, start, end) {
                ...
                var keys = this.indexToKeys(start, end);
                var codeIndex;
                if (code) {
            ...
            __kernelGetFormatCodeIndex: function(code) {
                var index;
                if (FORAMT_CODE_INDEX_MAP[code]) {
                    return FORAMT_CODE_INDEX_MAP[code];
                }
                index = "_" + this.data.customCodeIndex;
                this.data.customCodeIndex++;
                FORAMT_CODE_INDEX_MAP[code] = index;
    ...
    value: function(require) {
        ...
        return _p.r(0).create("Painter", {
            ...
            draw: function(canvas, struct, cellGrid) {
                ...
                ContentFillPainter.draw(canvas, layout);
                CellFillPainter.draw(canvas, fillCells);
                GridPainter.draw(canvas, struct);
                BorderPainter.draw(canvas, struct, cellGrid.cells);
                MergeFillPainter.draw(canvas, fillCells);
                ContentPainter.draw(canvas, layout);
        ...
        return {
            draw: function(canvas, layout) {
                ...
                for (var i = 0, len = layout.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = rowLayout.length; j < jlen; j++) {
                        currentLayout = rowLayout[j];
                        if (currentLayout.type === "mergecell" || currentLayout.wraptext) {
                            continue;
                        }
                        ...
                        y = box.y - CONTENT_PADDING;
                        canvas.save();
                        canvas.rect(x, y, width, height);
                        canvas.clip();
                        canvas.fillStyle(CANVAS_FILL_COLOR);
                        canvas.fillRect(x + box.offset, y, box.contentWidth + DOUBLE_PADING, height);
                        canvas.restore();
                    }
                }
            }
            ...
            draw: function(canvas, layout) {
                ...
                for (var i = 0, len = layout.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = rowLayout.length; j < jlen; j++) {
                        ...
                        if (currentLayout.type === "mergecell") {
                            drawMergeCellContent(canvas, currentLayout);
                        } else {
                            drawCellContent(canvas, currentLayout);
                        }
                        canvas.restore();
                    }
                }
...
_p[172] = {
    value: function(require) {
        var $ = _p.r(2);
        var CELL_VALUE_TYPE = _p.r(71);
        var BOOLEAN_VALUE = _p.r(74);
        return _p.r(0).create("Cell", {
            base: _p.r(94),
            init: function() {
                this.initService();
