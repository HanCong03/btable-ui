        ...
        var Actuary = _p.r(0).create("Actuary", {
            ...
            getNormalGrid: function(paneGrid) {
                ...
                if (paneGrid.rowIndex === -1) {
                    ...
                    spaceOffset.row = paneGrid.row[paneGrid.rowIndex + 1] - offset;
                ...
                if (paneGrid.colIndex === -1) {
                    ...
                    spaceOffset.col = paneGrid.col[paneGrid.colIndex + 1] - offset;
            ...
            __patchRow: function(grid) {
                var info = this.__getRowPatchInfo(grid.rowOverflow - 20, grid.startRow - 1);
                if (!info) {
                    return;
                }
            ...
            __patchCol: function(grid) {
                var info = this.__getColPatchInfo(grid.colOverflow - 20, grid.startCol - 1);
                if (!info) {
                    return;
                }
    ...
    value: function(require) {
        ...
        return _p.r(0).create("DataAccess", $.extend({
            ...
            setViewStartRow: function(row) {
                this.data.viewStart.row = row;
            },
            setViewStartCol: function(col) {
                this.data.viewStart.col = col;
            },
    ...
    value: function(require) {
        ...
        function exportCellInfo(data) {
            ...
            for (var key in valueData) {
                ...
                result[row][col] = valueData[key];
            }
            for (var i = 0, len = cellStyle.length; i < len; i++) {
                rowStyle = cellStyle[i];
                for (var j = 0, jlen = rowStyle.length; j < jlen; j++) {}
            }
        }
...
_p[101] = {
    value: function(require) {
        ...
        function importExcel(excelData) {
            ...
            if (excelData.sheetView.pane) {
                result.pane = {
                    row: excelData.sheetView.pane.r,
                    col: excelData.sheetView.pane.c
                };
            }
            ...
            result.allStyle = columnInfo.allStyle;
    ...
    value: function(require) {
        ...
        return {
            ...
            __kernelGetStyle: function(classify, row, col) {
                ...
                // 检查全局样式
                if (data.allStyle !== -1) {
                    return this.__kernelGetClassifyInStyle(classify, styleIndex[data.allStyle]);
                }
                return undefined;
            },
    ...
    value: function(require) {
        ...
        var Input = _p.r(0).create("Input", {
            ...
            onmousedown: function(row, col) {
                ...
                this.rs("c.range.focus", grid.rowMap[row], grid.colMap[col]);
            },
            lock: function() {
                this.lockState = true;
            },
            ...
            change: function() {
                ...
                if (mergeCell) {
                    this.updateMergeInput(mergeCell.start, mergeCell.end);
                } else {
                    this.updateInput(row, col);
                }
            },
            updateInput: function(row, col) {
                this.hideLabel();
                var struct = this.rs("c.struct");
                var grid = struct.getGrid();
                this.toggleToUninput();
                ...
                this.index.row = row;
                this.index.col = col;
                var style = struct.getAllComputedStyle(row, col);
                this.resetStyle(style);
                this.__relocation(grid.viewColPoints[c], grid.viewRowPoints[r]);
                //this.resetShadowWrapper(struct.getBorderWidth(), rect);
                this.resize(struct.getBorderWidth(), grid.viewWidth[c], grid.viewHeight[r]);
                this.enable();
                this.focus();
            },
            updateMergeInput: function() {
                debugger;
            },
            ondblclick: function() {
                var struct = this.rs("c.struct");
...
_p[156] = {
    value: function(require) {
        ...
        var InputShadowMan = _p.r(152);
        var TextHelper = _p.r(154);
        var EventManager = _p.r(155);
        var InputWorker = _p.r(159);
        var Input = _p.r(0).create("Input", {
            coverModule: null,
            eventManager: null,
            inputWorker: null,
            ...
            init: function(coverModule) {
                ...
                this.eventManager = this.createComponent(EventManager, this);
                this.inputWorker = this.createComponent(InputWorker, this);
                this.initEvent();
            },
            initEvent: function() {
        ...
        return {
            resize: function() {
                ...
                if (contentRect.width + SPACE_SIZE + HORIZONTAL_BUFFER_SIZE >= rect.width) {
                    ...
                    // update panel
                    this.sizePanel.style.width = newWidth + "px";
            ...
            calculateNewWidth: function(rect, contentRect) {
                ...
                var newWidth = viewWidths[activeCol];
                while (true) {
                    nextCol++;
                    // 没有后续单元格可用
            ...
            calculateNewHeight: function(rect, contentRect) {
                ...
                var newHeight = viewHeights[activeRow];
                while (true) {
                    nextRow++;
                    // 没有后续单元格可用
...
//src/system/input2/worker-control.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
_p[159] = {
    value: function(require) {
        ...
        return _p.r(0).create("InputWorker", $.extend({
            ...
            initEvent: function() {
                var _self = this;
                this.$inputNode.on("blur", function() {}).on("input", function() {
                    //if (_self.isFormulaModel()) {
                    //
                    //    _self.exitInput();
                    //    _self.offline();
                    ...
                    _self.contentchanged = true;
                    if (!this.writeModel) {
                        _self.shortcutWriteModel = true;
                        _self.__toActive(_self.activeCell.row, _self.activeCell.col);
                    }
                    _self.resize();
                }).on("keydown", function(evt) {
                    //if ((evt.ctrlKey || evt.metaKey) && evt.keyCode === 65) {
            ...
            toActive: function(r, c) {
                var struct = this.rs("c.struct");
                var grid = struct.getGrid();
                this.__toActive(grid.rMap[r], grid.cMap[c]);
            },
            __toActive: function(row, col) {
            ...
            write: function() {
                ...
                this.contentchanged = false;
                console.log("执行写入: " + this.inputNode.innerText);
            },
            clearInput: function() {
                this.$inputNode.html("&#xfeff;");
            },
...
//src/system/screen/body/common/pane-line-painter.js
/**
 * @file 网格绘制
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/normal/body-screen.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[165] = {
    value: function(require) {
        var Canvas = _p.r(185);
        var CellGrid = _p.r(166);
        var Painter = _p.r(170);
        return _p.r(0).create("BodyScreen", {
            panel: null,
            width: 0,
            height: 0,
...
//src/system/screen/body/normal/cell-grid.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/normal/line-painters/border.js
/**
 * @file 边框绘制
 * @author hancong03@baiud.com
 */
    ...
    value: function(require) {
        ...
        function drawBorder(canvas, contentCanvas, struct, borderIndex, rowIndex, colIndex) {
            ...
            if (borderOption.top !== NONE) {
                ...
                canvas.putImageData(imageData, grid.viewColPoints[colIndex] - BORDER_OFFSET, grid.viewRowPoints[rowIndex] - BORDER_OFFSET);
            }
            if (borderOption.right !== NONE) {
                imageData = getBorderImage(contentCanvas, borderIndex, borderOption, "right", height, boundary);
                canvas.putImageData(imageData, grid.viewColPoints[colIndex + 1] - BORDER_OFFSET, grid.viewRowPoints[rowIndex] - BORDER_OFFSET);
            }
            if (borderOption.bottom !== NONE) {
                imageData = getBorderImage(contentCanvas, borderIndex, borderOption, "bottom", width, boundary);
                canvas.putImageData(imageData, grid.viewColPoints[colIndex] - BORDER_OFFSET, grid.viewRowPoints[rowIndex + 1] - BORDER_OFFSET);
            }
...
//src/system/screen/body/normal/line-painters/box.js
/**
 * @file 整个内容空间框线绘制
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/normal/line-painters/grid.js
/**
 * @file 网格绘制
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/normal/painter.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[170] = {
    value: function(require) {
        var ContentFillPainter = _p.r(172);
        var ContentPainter = _p.r(173);
        var MergeFillPainter = _p.r(174);
        var CellFillPainter = _p.r(171);
        var GridPainter = _p.r(169);
        var BorderPainter = _p.r(167);
        var BoxPainter = _p.r(168);
        var PaneLinePainter = _p.r(164);
        return {
            draw: function(canvas, contentCanvas, struct) {
...
//src/system/screen/body/normal/painters/cell-fill.js
/**
 * @file¬
 * @author hancong03@baiud.com
 */
    ...
    value: function(require) {
        ...
        function getMergeCellInfo(struct) {
            ...
            for (var i = 0, len = visibleCount.row; i < len; i++) {
                for (var j = 0, jlen = visibleCount.col; j < jlen; j++) {
                    mergeInfo = struct.getMergeCell(grid.rowMap[i], grid.colMap[j]);
                    if (!mergeInfo) {
                        continue;
                    }
...
//src/system/screen/body/normal/painters/content-fill.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/normal/painters/content.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/normal/painters/merge-fill-painter.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
_p[175] = {
    value: function(require) {
        var NormalPainter = _p.r(170);
        var PanePainter = _p.r(180);
        return _p.r(0).create("Painter", {
            draw: function(canvas, contentCanvas, struct) {
                var pane = struct.getPane();
                if (pane.row === -1 && pane.col === -1) {
                    NormalPainter.draw(canvas, contentCanvas, struct);
                    return;
                }
                var viewStart = struct.getViewStart();
                ...
                PanePainter.draw(canvas, contentCanvas, struct);
            }
        });
    }
};

//src/system/screen/body/pane/cell-grid.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/pane/line-painters/border.js
/**
 * @file 边框绘制
 * @author hancong03@baiud.com
 */
_p[177] = {
    value: function(require) {
        ...
        var NONE = _p.r(74);
        return {
            draw: function(canvas, contentCanvas, struct, regions) {
                var region;
                for (var i = 0, len = regions.length; i < len; i++) {
                    ...
                    canvas.clip();
                    render(canvas, contentCanvas, struct, region);
                    canvas.restore();
                }
                // clear cache
                CACHE = {};
        ...
        function render(canvas, contentCanvas, struct, region) {
            var grid = struct.getGrid();
            var borders = struct.getBorders({
            ...
            drawBorders(canvas, contentCanvas, struct, borders, region);
        }
        function drawBorders(canvas, contentCanvas, struct, borders, region) {
            var rowBorders;
            var current;
            for (var i = 0, r = region.startRow, len = borders.length; i < len; i++, r++) {
                rowBorders = borders[i];
                for (var j = 0, c = region.startCol, jlen = rowBorders.length; j <= jlen; j++, c++) {
                    current = rowBorders[j];
                    if (current === undefined) {
                        continue;
                    }
                    drawBorder(canvas, contentCanvas, struct, current, r, c);
                }
            }
        }
        function drawBorder(canvas, contentCanvas, struct, borderIndex, rowIndex, colIndex) {
...
//src/system/screen/body/pane/line-painters/box.js
/**
 * @file 整个内容空间框线绘制
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/pane/line-painters/grid.js
/**
 * @file 网格绘制
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/pane/painter.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[180] = {
    value: function(require) {
        var ContentFillPainter = _p.r(182);
        var ContentPainter = _p.r(183);
        var MergeFillPainter = _p.r(184);
        var CellFillPainter = _p.r(181);
        var GridPainter = _p.r(179);
        var BorderPainter = _p.r(177);
        var BoxPainter = _p.r(178);
        var PaneLinePainter = _p.r(164);
        return {
            draw: function(canvas, contentCanvas, struct) {
                ...
                var imageData = contentCanvas.dumpImageData();
                //ContentFillPainter.draw(canvas, struct, contentLayout);
                CellFillPainter.draw(canvas, struct, regions);
                BorderPainter.draw(canvas, contentCanvas, struct, regions);
                contentCanvas.putImageData(imageData, 0, 0);
                GridPainter.draw(canvas, struct);
...
//src/system/screen/body/pane/painters/cell-fill.js
/**
 * @file¬
 * @author hancong03@baiud.com
 */
...
//src/system/screen/body/pane/painters/content-fill.js
/**
 * @file
 * @author hancong03@baiud.com
 */
        ...
        return {
            draw: function(canvas, struct, contentLayout) {
                ...
                for (var i = 0, len = 1; i < len; i++) {
                    ...
                    canvas.clip();
                    render(canvas, struct, currentLayout);
                    canvas.restore();
                }
            }
        };
        function render(canvas, struct, contentLayout) {
            var grid = struct.getGrid();
            var rowPoints = grid.row;
            ...
            var startX;
            var endX;
            canvas.fillStyle("red" || CANVAS_COLOR);
            for (var i = 0, len = contentLayout.length; i < len; i++) {
                rowLayout = contentLayout[i];
                for (var j = 0, jlen = rowLayout.length; j < jlen; j++) {
                    layout = rowLayout[j];
...
//src/system/screen/body/pane/painters/content.js
/**
 * @file
 * @author hancong03@baiud.com
 */
        ...
        return {
            draw: function(canvas, struct) {
                ...
                for (var i = 0, len = regions.length; i < len; i++) {
                    ...
                    canvas.rect(region.x, region.y, region.width, region.height);
                    canvas.clip();
                    renderCell(canvas, struct, layout[i]);
                    canvas.restore();
                }
                return cellInfo;
            }
...
//src/system/screen/body/pane/painters/merge-fill-painter.js
/**
 * @file
 * @author hancong03@baiud.com
 */
...
_p[187] = {
    value: function(require) {
        var BodyScreen = _p.r(163);
        var TopScreen = _p.r(188);
        var LeftScreen = _p.r(186);
...
_p[189] = {
    value: function(require) {
        ...
        function getWidthInfo(struct, startCol, endCol) {
            ...
            return {
                width: width + (widths.length - 1) * LINE_WIDTH,
                widthList: widths
            };
        }
        function getHeightInfo(struct, startRow, endRow) {
            ...
            return {
                height: height + (heights.length - 1) * LINE_WIDTH,
                heightList: heights
            };
        }
    }
...
_p[191] = {
    value: function(require) {
        ...
        var ScrollEventManager = _p.r(190);
...
_p[194] = {
    value: function(require) {
        ...
        var LINE_OFFSET = SYS_CONFIG.border.offset;
        return {
            getRect: function(r1, c1, r2, c2) {
                var struct = this.rs("c.struct");
                var grid = struct.getGrid();
                var colPoints = grid.viewColPoints;
                var rowPoints = grid.viewRowPoints;
                var leftCol = Math.min(c1, c2);
                var rightCol = Math.max(c1, c2) + 1;
                var topRow = Math.min(r1, r2);
                var bottomRow = Math.max(r1, r2) + 1;
                var width = colPoints[rightCol] - colPoints[leftCol] + LINE_WIDTH;
                var height = rowPoints[bottomRow] - rowPoints[topRow] + LINE_WIDTH;
                return {
                    x: colPoints[leftCol] - LINE_OFFSET,
                    y: rowPoints[topRow] - LINE_OFFSET,
                    width: width,
                    height: height
                };
            }
...
//src/system/selection2/selection.js
/**
 * @file
 * @author hancong03@baiud.com
 */
_p[195] = {
    value: function(require) {
        var $ = _p.r(2);
        var Selection = _p.r(0).create("Selection", $.extend({
            coverModule: null,
            selectionNode: null,
            $selectionNode: null,
            visiableState: false,
            ...
            onmousemove: function(r, c) {
                ...
                var rect = this.getRect(activeCell.r, activeCell.c, r, c);
                this.resize(rect);
            },
            onmouseup: function(r, c) {
                var activeCell = this.activeCell;
                if (r === activeCell.r && c === activeCell.c) {
                    this.hide();
                    return;
