    ...
    value: function(require) {
        ...
        return _p.r(0).create("Painter", {
            ...
            draw: function(canvas, struct, cellGrid) {
                ...
                var contentLayout = ContentPainter.draw(canvas, layout);
        ...
        return {
            draw: function(canvas, contentLayout) {
                ...
                for (var i = 0, len = contentLayout.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = rowLayout.length; j < jlen; j++) {
                        layout = rowLayout[j];
                        canvas.fillRect(layout.x, layout.y - CONTENT_PADDING, layout.width, layout.height + DOUBLE_PADING);
                    }
                }
                canvas.restore();
            }
            ...
            draw: function(canvas, layout) {
                ...
                for (var i = 0, len = layout.length; i < len; i++) {
                    ...
                    for (var j = 0, jlen = rowLayout.length; j < jlen; j++) {
                        ...
                        canvas.clearRect(currentLayout.box.cellX, currentLayout.box.cellY, 1e7, 1e7);
                        // mergecell 不用参与contentLayout的构建过程
                        if (currentLayout.type === "mergecell") {
                            drawMergeCellContent(canvas, currentLayout);
                        } else {
                            currentContentLayout.push(drawCellContent(canvas, currentLayout));
                        }
                    }
                    contentLayout.push(currentContentLayout);
                }
                canvas.restore();
